...
```
scripts/merge_branches.py --src cairo-v0.12.2
```
"""
...
FINAL_BRANCH = "main"
MERGE_PATHS_FILE = "scripts/merge/merge_paths.json"
NULL = "null"
...
def get_dst_branch(src_branch: str, dst_branch_override: Optional[str]) -> str:
    ...
    dst_branch = load_merge_paths()[src_branch]
    assert dst_branch != NULL, f"Default merge path is disabled for {src_branch}."
    return dst_branch
...
def merge_branches(src_branch: str, dst_branch: Optional[str]):
    ...
    if len(conflicts) != 0:
        compare = "https://github.com/starkware-industries/starkware/compare"
