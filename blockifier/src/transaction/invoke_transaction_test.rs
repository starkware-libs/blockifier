...
fn test_invoke_tx() -> TransactionExecutionResult<()> {
    ...
    let expected_validate_info = CallInfo {
        call: CallEntryPoint {
            class_hash: ClassHash(shash!(ACCOUNT_CONTRACT_CLASS_HASH)),
            entry_point_type: EntryPointType::External,
            entry_point_selector: EntryPointSelector(shash!(VALIDATE_ENTRY_POINT_SELECTOR)),
            calldata: tx.calldata.clone(),
            storage_address: ContractAddress::try_from(shash!(ACCOUNT_CONTRACT_ADDRESS))?,
        },
    ...
    let expected_execute_call = CallEntryPoint {
        ...
        ..expected_validate_info.call.clone()
    };

    let tx_calldata = tx.calldata.0;
    let expected_execute_info = CallInfo {
    ...
    let expected_result = TransactionExecutionInfo {
        validate_info: expected_validate_info,
        execute_info: Some(expected_execute_info),
        fee_transfer_info: CallInfo::default(),
        actual_fee: get_tested_actual_fee(),
        actual_resources: ResourcesMapping::default(),
    };

    assert_eq!(execute_result, expected_result);
