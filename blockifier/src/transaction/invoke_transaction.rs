...
use crate::transaction::transaction_utils::{
    calculate_tx_fee, execute_fee_transfer, get_contract_class, verify_tx_version,
};
...
impl ExecuteTransaction for InvokeTransaction {
    fn execute(
        ...
        let validate_info = validate_call.execute(state)?;
        ...
        // TODO(Adi, 20/12/2022): Replace with `state` once `execute` gets a reference to state.
        let execute_info = execute_call.execute(CachedState::default())?;
