...
use starknet_api::{
    CallData, EntryPointSelector, EntryPointType, Fee, Nonce, StarkHash, TransactionHash,
...
impl ExecuteTransaction for InvokeTransaction {
    fn execute(&self) -> Result<TransactionExecutionInfo, TransactionExecutionError> {
        ...
        let call = CallEntryPoint::new(
            ...
            EntryPointSelector(StarkHash::from_hex(VALIDATE_ENTRY_POINT_SELECTOR)?),
        ...
        let call = CallEntryPoint::new(
            contract_class,
            EntryPointType::External,
            EntryPointSelector(StarkHash::from_hex(EXECUTE_ENTRY_POINT_SELECTOR)?),
            self.calldata.clone(),
        );
