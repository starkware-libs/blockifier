...
impl<SR: StateReader> CachedState<SR> {
    ...
    pub fn get_contract_class(
        ...
        if !self.contract_class_cache.contains_key(contract_hash) {
            ...
            match contract_class {
                ...
                None => return Ok(None),
                Some(contract_class) => {
                    self.contract_class_cache.insert(*contract_hash, Rc::clone(&contract_class));
                }
            }
        }
        Ok(Some(Rc::clone(self.contract_class_cache.get(contract_hash).unwrap())))
    }
}
