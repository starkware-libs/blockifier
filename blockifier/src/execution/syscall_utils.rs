...
use crate::execution::cairo_run_utils::get_bigint_from_memory_cell;
...
pub fn storage_read(
    ...
    let selector = get_bigint_from_memory_cell(vm.get_maybe(&syscall_ptr)?)?;
    let address = get_bigint_from_memory_cell(vm.get_maybe(&(&syscall_ptr + 1))?)?;
    ...
    vm.insert_value(&(&syscall_ptr + 2), response.value)?;
