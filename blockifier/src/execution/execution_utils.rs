...
use crate::execution::errors::{
    PostprocessExecutionError, PreprocessExecutionError, RunEntryPointError,
};
...
) -> Result<CairoRunObjects, PreprocessExecutionError> {
...
pub fn execute_call_entry_point(
    ...
    Ok(create_call_info(&run_objects.cairo_runner, &run_objects.vm, call_entry_point)?)
}
...
pub fn create_call_info(
    ...
    vm: &VirtualMachine,
    call_entry_point: &CallEntryPoint,
) -> Result<CallInfo, PostprocessExecutionError> {
    let retdata = extract_execution_return_data(vm)?;
    let execution = CallExecution { retdata };
    let syscall_handler = cairo_runner.exec_scopes.get_ref::<SyscallHandler>("syscall_handler")?;
    Ok(CallInfo {
        call: call_entry_point.clone(),
        execution,
        internal_calls: syscall_handler.internal_calls.clone(),
    })
}

fn extract_execution_return_data(
    vm: &VirtualMachine,
) -> Result<Vec<StarkFelt>, PostprocessExecutionError> {
