...
impl HintProcessor for SyscallHandler<'_> {
    fn execute_hint(
        &mut self,
        vm: &mut VirtualMachine,
        ...
        match self.builtin_hint_processor.execute_hint(vm, exec_scopes, hint_data, constants) {
            Err(VirtualMachineError::UnknownHint(hint)) if SYSCALL_HINTS.contains(&&hint[..]) => {
                let hint_data = hint_data
                    .downcast_ref::<HintProcessorData>()
                    .ok_or(VirtualMachineError::WrongHintData)?;
                self.execute_syscall(vm, &hint_data.ids_data, &hint_data.ap_tracking)
            }
            other => other,
        }
    }
...
) -> (Relocatable, SyscallHandler<'a>) {
    let syscall_segment = vm.add_memory_segment();
    let syscall_handler =
        SyscallHandler::new(syscall_segment, state, call_entry_point.storage_address);
...
pub fn execute_inner_call(
    call_entry_point: CallEntryPoint,
    syscall_handler: &mut SyscallHandler<'_>,
