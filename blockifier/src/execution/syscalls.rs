...
impl DeployRequest {
    ...
    pub fn execute_constructor_entry_point(
        ...
        if constructor_entry_points.is_empty() {
            ...
            let call_info =
                CallInfo::create_for_empty_constructor_call(self.class_hash, contract_address);
            syscall_handler.inner_calls.push(call_info);

            return Ok(SyscallResponse::Deploy(DeployResponse { contract_address }));
        } else {
            ...
            execute_inner_call(constructor_entry_point, syscall_handler)?;
        };
