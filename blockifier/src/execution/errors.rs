...
pub enum EntryPointExecutionError {
    #[error(transparent)]
    ExecutionPreparationError(#[from] ExecutionPreparationError),
    #[error(transparent)]
...
pub enum SyscallExecutionError {
    ...
    #[error(transparent)]
    ContractCallExecutionError(Box<EntryPointExecutionError>),
...
impl From<EntryPointExecutionError> for SyscallExecutionError {
    fn from(error: EntryPointExecutionError) -> Self {
        Self::ContractCallExecutionError(Box::new(error))
    }
}
...
impl From<RunnerError> for ExecutionPreparationError {
