...
fn test_entry_point_without_arg() -> Result<()> {
    let entry_point = CallEntryPoint {
        entry_point_selector: EntryPointSelector(StarkHash::try_from(WITHOUT_ARG_SELECTOR)?),
...
fn test_entry_point_with_arg() -> Result<()> {
    ...
    let entry_point = CallEntryPoint {
        calldata,
        entry_point_selector: EntryPointSelector(StarkHash::try_from(WITH_ARG_SELECTOR)?),
        ..trivial_external_entrypoint()
    };
    assert_eq!(entry_point.execute()?, Vec::<StarkFelt>::new());
    Ok(())
...
fn test_entry_point_with_builtin() -> Result<()> {
    ...
    let entry_point = CallEntryPoint {
        calldata,
        entry_point_selector: EntryPointSelector(StarkHash::try_from(BITWISE_AND_SELECTOR)?),
        ..trivial_external_entrypoint()
    };
    assert_eq!(entry_point.execute()?, Vec::<StarkFelt>::new());
    Ok(())
...
fn test_entry_point_with_hint() -> Result<()> {
    ...
    let entry_point = CallEntryPoint {
        calldata,
        entry_point_selector: EntryPointSelector(StarkHash::try_from(SQRT_SELECTOR)?),
        ..trivial_external_entrypoint()
    };
    assert_eq!(entry_point.execute()?, Vec::<StarkFelt>::new());
    Ok(())
...
fn test_entry_point_with_return_value() -> Result<()> {
    ...
    let entry_point = CallEntryPoint {
        calldata,
        entry_point_selector: EntryPointSelector(StarkHash::try_from(RETURN_RESULT_SELECTOR)?),
        ..trivial_external_entrypoint()
    };
...
fn test_entry_point_with_syscall() -> Result<()> {
    ...
    let entry_point = CallEntryPoint {
        calldata,
        entry_point_selector: EntryPointSelector(StarkHash::try_from(GET_VALUE_SELECTOR)?),
        ..trivial_external_entrypoint()
    };
