...
use crate::test_utils::{
    create_test_state_rc, BITWISE_AND_SELECTOR, GET_VALUE_SELECTOR, RETURN_RESULT_SELECTOR,
...
fn test_entry_point_with_syscall() {
    ...
    assert_eq!(
        entry_point.execute(state.clone()).unwrap().execution,
        CallExecution { retdata: vec![value] }
    );
    // Verify that the state has changed.
    let value_from_state =
        *state.borrow_mut().get_storage_at(storage_address, key.try_into().unwrap()).unwrap();
