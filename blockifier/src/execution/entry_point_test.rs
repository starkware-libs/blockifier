...
use crate::execution::{entry_point::CallEntryPoint, contract_class_raw::ContractClass};
...
fn test_entry_point_without_arg() -> Result<()> {
    ...
    let contract_class = ContractClass::from_file(&path)?.into();
    let entry_point = CallEntryPoint::new(&contract_class, TEST_CONTRACT_PROGRAM, "without_arg", vec![]);
...
fn test_entry_point_with_arg() -> Result<()> {
    let path = PathBuf::from(TEST_CONTRACT);
    let contract_class = ContractClass::from_file(&path)?.into();
    let entry_point = CallEntryPoint::new(&contract_class, TEST_CONTRACT_PROGRAM, "with_arg", vec![25]);
    assert_eq!(entry_point.execute()?, Vec::<BigInt>::new());
    Ok(())
}
...
fn test_entry_point_with_builtin() -> Result<()> {
    let path = PathBuf::from(TEST_CONTRACT);
    let contract_class = ContractClass::from_file(&path)?.into();
    let entry_point = CallEntryPoint::new(&contract_class, TEST_CONTRACT_PROGRAM, "bitwise_and", vec![47, 31]);
    assert_eq!(entry_point.execute()?, Vec::<BigInt>::new());
    Ok(())
}
...
fn test_entry_point_with_return_value() -> Result<()> {
    let path = PathBuf::from(TEST_CONTRACT);
    let contract_class = ContractClass::from_file(&path)?.into();
    let entry_point = CallEntryPoint::new(&contract_class, TEST_CONTRACT_PROGRAM, "return_result", vec![23]);
