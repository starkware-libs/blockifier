...
fn test_security_failure() {
    ...
    fn run_security_test(
        ...
        let error = match entry_point_call.execute_directly(state) {
            ...
            Ok(_) => panic!(
                "Entry point did '{entry_point_name}' not fail! Expected error: {expected_error}"
            ),
        };
    ...
    for perform_inner_call_to_foo in 0..2 {
        ...
        run_security_test(
            ...
            "test_relocateable_storage_address",
            Some(calldata),
            &mut state,
        );
    }

    let calldata = Calldata(vec![stark_felt!(1), stark_felt!(1)].into());
    run_security_test(
    ...
    run_security_test("Expected relocatable", "test_bad_syscall_stop_ptr2", None, &mut state);
    ...
    // TODO(AlonH, 21/12/2022): Uncomment after get_tx_signature syscall is implemented.
    // run_security_test(
    //     "Validation failed: Read-only segments",
    ...
    // TODO(AlonH, 21/12/2022): Uncomment after get_tx_info syscall is implemented.
    // run_security_test(
    //     "Validation failed: Read-only segments",
