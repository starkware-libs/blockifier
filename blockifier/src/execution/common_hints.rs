...
const NORMALIZE_ADDRESS_IS_SMALL_HINT: &str = r#"# Verify the assumptions on the relationship between 2**250, ADDR_BOUND and PRIME.
...
const NORMALIZE_ADDRESS_IS_250_HINT: &str = "ids.is_250 = 1 if ids.addr < 2**250 else 0";
...
pub fn normalize_address_is_small_deduction(
    ...
    let addr_bound_lower_bound = &bigint!(1).shl(250);
    let addr_bound_inclusive_upper_bound = &bigint!(1).shl(251);
    if addr_bound <= addr_bound_lower_bound
        || addr_bound > addr_bound_inclusive_upper_bound
        || prime <= &(bigint!(2) * addr_bound_lower_bound)
        || &(2 * addr_bound) <= prime
    {
...
///
/// Must comply with the API of a hint function, as defined by the `HintProcessor`.
pub fn normalize_address_is_250_deduction(
    vm: &mut VirtualMachine,
    _execution_scopes: &mut ExecutionScopes,
    ids_data: &HashMap<String, HintReference>,
    ap_tracking: &ApTracking,
...
pub fn add_common_hints(hint_processor: &mut BuiltinHintProcessor) {
    hint_processor.add_hint(
        String::from(NORMALIZE_ADDRESS_IS_SMALL_HINT),
        Rc::new(HintFunc(Box::new(normalize_address_is_small_deduction))),
    );
    hint_processor.add_hint(
        String::from(NORMALIZE_ADDRESS_IS_250_HINT),
        Rc::new(HintFunc(Box::new(normalize_address_is_250_deduction))),
    );
}
