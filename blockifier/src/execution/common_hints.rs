...
pub fn normalize_address_is_small_deduction(
    ...
    let addr_bound = &(addr_bound % prime);
    ...
    let is_small = if addr < Cow::Borrowed(addr_bound) { bigint!(1) } else { bigint!(0) };
...
pub fn normalize_address_is_250_deduction(
    ...
    let is_250 = if addr < Cow::Owned(bigint!(1).shl(250)) { bigint!(1) } else { bigint!(0) };
...
pub fn add_common_hints(hint_processor: &mut BuiltinHintProcessor) {
    ...
    hint_processor.add_hint(
        String::from("ids.is_250 = 1 if ids.addr < 2**250 else 0"),
