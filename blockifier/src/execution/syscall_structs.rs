...
impl StorageReadRequest {
    ...
    pub fn execute(&self, syscall_handler: &mut SyscallHandler) -> ExecutionResult {
        let contract_address = syscall_handler.outer_call.storage_address;
...
impl LibraryCallRequest {
    ...
    pub fn execute(&mut self, syscall_handler: &mut SyscallHandler) -> ExecutionResult {
        let storage_address = syscall_handler.outer_call.storage_address;
        ...
        // TODO(AlonH, 21/12/2022): Remove clone when `state` becomes a reference. Important note:
        // until then, outer state will not change in the inner call.
        ...
        Ok(SyscallResponse::LibraryCall(LibraryCallResponse {
            retdata_size: shash!(retdata.len() as u64),
            retdata,
        }))
    }
}
