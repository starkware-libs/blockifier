...
impl CallContractResponse {
    pub fn write(self, vm: &mut VirtualMachine, ptr: &Relocatable) -> WriteResponseResult {
        write_retdata(vm, ptr, self.retdata_size, self.retdata)
    }
}
...
impl DeployRequest {
    ...
    pub fn execute(&self, _syscall_handler: &mut SyscallHandler) -> ExecutionResult {
        ...
        Ok(SyscallResponse::Deploy(DeployResponse {
            ...
            constructor_retdata_size: StarkFelt::from(0),
            constructor_retdata: vec![StarkFelt::from(0)],
        }))
    }
}
...
pub struct DeployResponse {
    ...
    pub constructor_retdata_size: StarkFelt,
...
impl DeployResponse {
    pub fn write(self, vm: &mut VirtualMachine, ptr: &Relocatable) -> WriteResponseResult {
        ...
        write_retdata(vm, ptr, self.constructor_retdata_size, self.constructor_retdata)
    }
}
...
fn write_retdata(
    ...
    ptr: &Relocatable,
    retdata_size: StarkFelt,
    retdata: Vec<StarkFelt>,
) -> WriteResponseResult {
    vm.insert_value(ptr, felt_to_bigint(retdata_size))?;
...
fn read_calldata(
    vm: &VirtualMachine,
    ptr: &Relocatable,
) -> Result<CallData, SyscallExecutionError> {
