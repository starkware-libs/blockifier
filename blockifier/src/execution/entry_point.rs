...
use starknet_api::ContractClass;
...
impl CallEntryPoint {
    pub fn new(contract_class: &ContractClass, contract_file_path: &str, name: &str, calldata: Vec<i32>) -> Self {
        let contract_file_path = PathBuf::from(contract_file_path);
        let name = name.to_string();
        Self {contract_class: contract_class.clone(), contract_file_path, name, calldata }
    }
