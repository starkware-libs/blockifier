...
impl CallEntryPoint {
    ...
    pub fn execute_directly(self, state: &mut dyn State) -> EntryPointExecutionResult<CallInfo> {
        self.execute(
            state,
            &BlockContext::get_test_block_context().unwrap(),
...
impl BlockContext {
    pub fn get_test_block_context() -> Result<BlockContext, StarknetApiError> {
        Ok(BlockContext {
            chain_id: ChainId("SN_GOERLI".to_string()),
            block_number: BlockNumber::default(),
            block_timestamp: BlockTimestamp::default(),
            sequencer_address: ContractAddress(patky!(TEST_SEQUENCER_ADDRESS)),
            fee_token_address: ContractAddress(patky!(TEST_ERC20_CONTRACT_ADDRESS)),
