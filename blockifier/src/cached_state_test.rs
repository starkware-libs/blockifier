...
fn get_contract_class() -> Result<()> {
    ...
    let class_hash_not_found = ClassHash(shash!("0x101"));
    let mut state = CachedState::new(DictStateReader {
        class_hash_to_class: HashMap::from([(class_hash, Rc::new(contract_class.clone()))]),
        ..Default::default()
    });

    assert_matches!(state.get_contract_class(&class_hash_not_found),
    Err(PreExecutionError::UndeclaredClassHash(class_hash))if class_hash == class_hash_not_found);
    assert_eq!(*state.get_contract_class(&class_hash).unwrap(), contract_class);
