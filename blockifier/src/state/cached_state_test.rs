...
fn cached_state_state_diff_conversion() {
    // contract_address0 will correspond to a keys that are not touched in the test.
    ...
    let contract_address0 = ContractAddress(patky!("0x100"));
    let contract_address1 = ContractAddress(patky!("0x200"));
    let contract_address2 = ContractAddress(patky!("0x300"));

    let nonce_2_raw = 5_u64;
    // Here contract_address1 is not changed, since you can't change it to the same value.
    let nonce_initial_values = HashMap::from([
        (contract_address0, Nonce::default()),
        (contract_address1, Nonce::default()),
        (contract_address2, Nonce(StarkFelt::from(nonce_2_raw))),
    ]);
    ...
    let key_x = StorageKey(patky!("0x10"));
    let key_y = StorageKey(patky!("0x20"));
    let storage_val0: StarkFelt = shash!("0x1");
    let storage_val1: StarkFelt = shash!("0x5");
    let storage_val2: StarkFelt = shash!("0x6");
    ...
    // In order to set a new ClassHash the previous must be the Default::default() one.
    let class_hash_0 = ClassHash(shash!("0x2000"));
    let address_to_class_hash_initial_values = HashMap::from([
        (contract_address0, class_hash_0),
        (contract_address1, ClassHash::default()),
        (contract_address2, ClassHash::default()),
    ]);

    let mut state = CachedState::new(DictStateReader {
        storage_view: storage_initial_values.clone(),
        address_to_nonce: nonce_initial_values.clone(),
        address_to_class_hash: address_to_class_hash_initial_values.clone(),
        class_hash_to_class: class_hash_to_class.clone(),
    });
    ...
    assert!(state.set_class_hash_at(contract_address1, ClassHash::default()).is_ok());
    ...
    assert!(state.increment_nonce(contract_address2).is_ok());
    let new_class_hash = ClassHash(shash!("0x11111111"));
    ...
    let expected_state_diff = StateDiff {
        ...
        nonces: IndexMap::from_iter([(
            contract_address2,
            Nonce(StarkFelt::from(nonce_2_raw + 1_u64)),
        )]),
