...
const CAIRO_CMD: &str = "cairo-compile";

// Checks that:
// * `TEST_CONTRACTS` dir exists and contains only `.cairo` files and the subdirectory
// `COMPILED_CONTRACTS_SUBDIR`.
// * for each `X.cairo` file in `TEST_CONTRACTS` there exists an `X.json` file in
// `COMPILED_CONTRACTS_SUBDIR` which equals `cairo-compile X.cairo -- -no_debug_info`.
#[test]
#[ignore]
fn compiled_files_compatibility() -> Result<()> {
    for file in fs::read_dir(FEATURE_CONTRACTS_DIR).unwrap() {
        ...
        let existing_compiled_path = format!("{}/{file_name}.json", compiled_contracts_path());
        ...
        let expected_compiled_output =
            Command::new(CAIRO_CMD).args([&path_str, "--no_debug_info"]).output().unwrap().stdout;
...
fn compiled_contracts_path() -> String {
    FEATURE_CONTRACTS_DIR.to_owned() + "/" + COMPILED_CONTRACTS_SUBDIR
}
