        ...
        loop {
            match resource_bounds.pop_front() {
                Option::Some(current_word) => {
                    assert(*tx_info_resource_bounds.pop_front().unwrap().resource == current_word.resource, 'RESOURCE_MISMATCH');
                    assert(*tx_info_resource_bounds.pop_front().unwrap().max_amount == current_word.max_amount, 'MAX_AMOUNT_MISMATCH');
                    assert(*tx_info_resource_bounds.pop_front().unwrap().max_price_per_unit == current_word.max_price_per_unit, 'MAX_PRICE_PER_UNIT_MISMATCH');
                },
                Option::None => { break; }
            }
        };
