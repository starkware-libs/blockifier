...
impl CallEntryPoint {
    ...
    pub fn resolve_entry_point_pc(
        ...
        // Return the default entrypoint if the given selector is missing.
        ...
        // At least two entry points with same selector.
        if filtered_entry_points.len() > 1 {
            return not_found_error;
        }

        // Filtered entry points contain exactly one element.
        match filtered_entry_points.get(0) {
            None => not_found_error, // Unreachable, but error is better than panic.
