...
impl EntryPointExecutionContext {
    ...
    fn max_steps(
        ...
        let block_upper_bound = match mode {
            ExecutionMode::Validate => {
                min(block_info.validate_max_n_steps as usize, constants::MAX_VALIDATE_STEPS_PER_TX)
            }
            ExecutionMode::Execute => {
                min(block_info.invoke_tx_max_n_steps as usize, constants::MAX_STEPS_PER_TX)
            }
        };
        ...
        let tx_gas_upper_bound = match account_tx_context {
            AccountTransactionContext::Deprecated(context) => {
                (context.max_fee.0
                    / block_info.gas_prices.get_by_fee_type(&account_tx_context.fee_type()))
                    as usize
            }
