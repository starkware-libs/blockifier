...
use cairo_felt::{Felt, FeltOps, PRIME_STR};
...
pub fn normalize_address_set_is_small(
    ...
    let addr_bound_felt =
        constants.get(ADDR_BOUND).ok_or(HintError::MissingConstant("ADDR_BOUND"))?;
    let addr = get_integer_from_var_name("addr", vm, ids_data, ap_tracking)?;

    // Move to big int representation, as prime = 0 in the prime field.
    let prime = BigInt::from_str_radix(&PRIME_STR[2..], 16)
        .map_err(|_| VirtualMachineError::CouldntParsePrime(PRIME_STR.to_string()))?;
    let addr_bound = &addr_bound_felt.to_bigint().mod_floor(&prime);
    if !(addr_bound > &BigInt::from(1).shl(250)
        && addr_bound <= &BigInt::from(1).shl(251)
        && prime > (BigInt::from(2) * BigInt::from(1).shl(250))
        && (2 * addr_bound) > prime)
    {
    ...
    let is_small = if *addr < *addr_bound_felt { Felt::one() } else { Felt::zero() };
