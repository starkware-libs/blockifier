...
fn run_security_test(expected_error: &str, entry_point_name: &str, calldata: Calldata) {
...
fn test_vm_execution_security_failures() {
    run_security_test("Expected relocatable", "test_nonrelocatable_syscall_ptr", calldata![]);
    run_security_test("Unknown value for memory cell", "test_unknown_memory", calldata![]);
    run_security_test(
    ...
    run_security_test("Cannot exit main scope.", "test_exit_main_scope", calldata![]);
    run_security_test(
...
fn test_builtin_execution_security_failures() {
    ...
    run_security_test("Signature hint is missing", "test_missing_signature_hint", calldata![]);
    run_security_test(
    ...
    run_security_test("is not on the curve", "test_ec_op_point_not_on_curve", calldata![]);
}
...
fn test_syscall_execution_security_failures() {
    for perform_inner_call_to_foo in 0..2 {
        ...
        run_security_test("Hint Error: Out of range", "test_read_bad_address", calldata.clone());
