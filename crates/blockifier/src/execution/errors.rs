...
pub fn match_transaction_execution_error(error: &TransactionExecutionError) -> String {
    let mut error_stack: Vec<String> = Vec::new();

    match error {
        TransactionExecutionError::ExecutionError{ error, storage_address } | TransactionExecutionError::ValidateTransactionError { error, storage_address } | TransactionExecutionError::ContractConstructorExecutionFailed { error, storage_address } => {
            error_stack.push(format!("Error in the called contract ({}):", storage_address));
            error_stack = match_entry_point_execution_error(error_stack, error);
        }
        _ => {
...
pub fn match_execute_directly(error: &CairoRunError, storage_address: StarkFelt) -> String {
    let mut error_stack: Vec<String> = Vec::new();
    error_stack.push(format!("Error in the called contract ({}):", storage_address));
    error_stack = match_cairo_run_error(error_stack, error);
    ...
    if error_stack.last().unwrap_or(&String::new()).starts_with("Execution failed. Failure reason:") {
        let last_wrapping_hint_error_index = error_stack.iter().rposition(|s| s.starts_with("Got an exception while executing a hint."));
        if last_wrapping_hint_error_index.is_some() {
            let index = last_wrapping_hint_error_index.unwrap();
            // replace error stack at index with the cairo 1 error that appears at the end of the stack.
            error_stack[index] = format!("{HINT_ERROR_STR}{}", error_stack.last().unwrap().trim_end());
        }
    }
...
fn match_cairo_run_error(mut error_stack: Vec<String>, error: &CairoRunError) -> Vec<String>{
    if let CairoRunError::VmException(vm_exception) = error {
        return match_vm_exception(error_stack, vm_exception);
    }
    error_stack.push(error.to_string());
    error_stack
}

fn match_vm_exception(mut error_stack: Vec<String>, vm_exception: &VmException) -> Vec<String> {
    let vm_exception_preemble = format!("Error at pc=0:{}:", vm_exception.pc);
    error_stack.push(vm_exception_preemble);

    // This match us temporarily here to match existing regression. This is to be deleted shortly.
    match &vm_exception.inner_exc {
        VirtualMachineError::Hint(_) => {
            error_stack.push(String::from("Got an exception while executing a hint."));
        }
    ...
    match_virtual_machine_error(error_stack, &vm_exception.inner_exc)
}

fn match_virtual_machine_error(mut error_stack: Vec<String>, vm_error: &VirtualMachineError) -> Vec<String>{
    match vm_error {
        VirtualMachineError::Hint(ref bx) => {
            if let HintError::Internal(internal_vm_error) = &bx.1 {
                return match_virtual_machine_error(error_stack, internal_vm_error);
            }
            error_stack.push(bx.1.to_string());
        }
        VirtualMachineError::Other(anyhow_error) => {
            let syscall_exec_err = anyhow_error.downcast_ref::<SyscallExecutionError>();
            if syscall_exec_err.is_some() {
                let downcast_anyhow = syscall_exec_err.unwrap();
                return match_syscall_execution_error(error_stack, downcast_anyhow);
            }
            else {
                let deprecated_syscall_exec_err = anyhow_error.downcast_ref::<DeprecatedSyscallExecutionError>();
                if deprecated_syscall_exec_err.is_some() {
                    let downcast_anyhow = deprecated_syscall_exec_err.unwrap();
                    return match_deprecated_syscall_execution_error(error_stack, downcast_anyhow);
                }
            }
        }
        _ => {}
    ...
    // This default push should be reinstated shortly.
    // error_stack.push(format!("{}\n", vm_error.to_string()));
    error_stack
}

fn match_syscall_execution_error(mut error_stack: Vec<String>, syscall_error: &SyscallExecutionError) -> Vec<String>{
    match syscall_error {
        SyscallExecutionError::CallContractExecutionError { storage_address, error } => {
            let call_contract_preemble = format!("Error in the called contract ({}):", storage_address.0.key());
            error_stack.push(call_contract_preemble);
            return match_syscall_execution_error(error_stack, error);
        }
        SyscallExecutionError::LibraryCallExecutionError { storage_address, error, .. } => {

            // TODO: Change to this, or a similar string that includes the class hash.
            // let libcall_preemble = format!("Error in the called contract (hash: {} storage: {}):", class_hash, storage_address.0.key());
            let libcall_preemble = format!("Error in the called contract ({}):", storage_address.0.key());
            error_stack.push(libcall_preemble);
            return match_syscall_execution_error(error_stack, error);
        }
        SyscallExecutionError::EntryPointExecutionError(entry_point_error) => {
            return match_entry_point_execution_error(error_stack, entry_point_error);
        }
        _ => {
...
fn match_deprecated_syscall_execution_error(mut error_stack: Vec<String>, syscall_error: &DeprecatedSyscallExecutionError) -> Vec<String>{
    match syscall_error {
        DeprecatedSyscallExecutionError::CallContractExecutionError { storage_address, error } => {
            let call_contract_preemble = format!("Error in the called contract ({}):", storage_address.0.key());
            error_stack.push(call_contract_preemble);
            return match_deprecated_syscall_execution_error(error_stack, error);
        }
        DeprecatedSyscallExecutionError::LibraryCallExecutionError { storage_address, error , ..} => {
...
fn match_entry_point_execution_error(mut error_stack: Vec<String>, entry_point_error: &EntryPointExecutionError) -> Vec<String>{
    match entry_point_error {
        EntryPointExecutionError::VirtualMachineExecutionErrorWithTrace { source, .. } => {
            return match_cairo_run_error(error_stack, source);
        }
        _ => {
            error_stack.push(format!("{}\n", entry_point_error));
        }
