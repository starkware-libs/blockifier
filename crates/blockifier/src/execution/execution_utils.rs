...
pub fn prepare_call_arguments(
...
) -> Result<(Vec<MaybeRelocatable>, Vec<CairoArg>), PreExecutionError> {
    ...
    args.push(CairoArg::Single(entry_point_selector));
    ...
    args.push(CairoArg::Array(implicit_args.clone()));
    ...
    args.push(CairoArg::Single(MaybeRelocatable::from(calldata.len())));
    let calldata_start_ptr = MaybeRelocatable::from(read_only_segments.allocate(vm, calldata)?);
    args.push(CairoArg::Single(calldata_start_ptr));
...
pub fn run_entry_point(
    ...
    cairo_runner.run_from_entrypoint(entry_point_pc, &args[..], true, vm, hint_processor)?;
    Ok(())
}
