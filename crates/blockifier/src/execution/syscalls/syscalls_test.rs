...
use crate::execution::syscalls::{
    SYSCALL_MAX_EVENTS_COUNT, SYSCALL_MAX_EVENT_DATA, SYSCALL_MAX_EVENT_KEYS,
};
use crate::retdata;
...
fn test_emit_event() {
    ...
    let number_events = vec![stark_felt!(1_u16)];
    let call_info = emit_events(&number_events, &keys, &data).unwrap();
    ...
    let error = emit_events(&number_events, &keys, &data_too_long).unwrap_err();
    let expected_error = EmitEventError::ExceedsDataLength {
    ...
    let error = emit_events(&number_events, &keys_too_long, &data).unwrap_err();
    let expected_error = EmitEventError::ExceedsKeyLength {
        key_length: SYSCALL_MAX_EVENT_KEYS + 1,
        max_key_length: SYSCALL_MAX_EVENT_KEYS,
    };
    assert!(error.to_string().contains(format!("{}", expected_error).as_str()));

    // Negative flow, the number of events exceeds the limit.
    let number_events_too_big = vec![stark_felt!((SYSCALL_MAX_EVENTS_COUNT + 1) as u16)];
    let error = emit_events(&number_events_too_big, &keys, &data).unwrap_err();
    let expected_error = EmitEventError::ExceedsNumberOfEvents {
        number_events: SYSCALL_MAX_EVENTS_COUNT + 1,
