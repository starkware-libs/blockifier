...
fn verify_compiler_version(file_path: &str, expected_version: &str) {
    ...
    let json_value: serde_json::Value = serde_json::from_str(&content).expect("Error parsing JSON");

    // Access the value of "compiler_version"
    if let Some(compiler_version) = json_value.get("compiler_version") {
        dbg!(compiler_version);
        if let Some(compiler_version_str) = compiler_version.as_str() {
            assert_eq!(compiler_version_str, expected_version);
        } else {
            panic!("'compiler_version' is not a valid string: {:?}", compiler_version);
        }
    } else {
        panic!("'compiler_version' not found in JSON.");
    }
}
...
fn test_secp256k1() {
    ...
    assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 32616270_u64, ..Default::default() }
    );
}
...
fn test_secp256r1() {
    ...
    assert_eq!(
        entry_point_call.execute_directly(&mut state).unwrap().execution,
        CallExecution { gas_consumed: 48722700_u64, ..Default::default() }
    );
}
