    ...
    fn allocate_tx_info_segment(&mut self, vm: &mut VirtualMachine) -> SyscallResult<Relocatable> {
        ...
        match account_tx_context {
            AccountTransactionContext::Current(current_ctx) => {
                ...
                let tx_paymaster_data_start_ptr =
                ...
                let stark_felt_size_bytes = std::mem::size_of::<StarkFelt>();
                let element_size_bytes = stark_felt_size_bytes * tx_paymaster_data_length;
                let tx_paymaster_data_end_ptr =
                    (*tx_paymaster_data_start_ptr + element_size_bytes)?;

                let tx_account_deployment_data_start_ptr = &self
            ...
            AccountTransactionContext::Deprecated(_) => {
                ...
                let empty_array_ptr = &self.allocate_data_segment(vm, Vec::<StarkFelt>::new())?;
