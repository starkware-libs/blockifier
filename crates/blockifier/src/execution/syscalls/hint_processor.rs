...
impl<'a> SyscallHintProcessor<'a> {
    ...
    fn allocate_tx_info_segment(&mut self, vm: &mut VirtualMachine) -> SyscallResult<Relocatable> {
        ...
        match account_tx_context {
            ...
            AccountTransactionContext::Deprecated(_) => {
                tx_info.extend_from_slice(&[
                    Felt252::zero().into(), // Empty list of ResourceBound
                    Felt252::zero().into(),
                    Felt252::zero().into(), // Tip
                    Felt252::zero().into(), // Empty list of paymaster_data
                    Felt252::zero().into(),
                    Felt252::zero().into(), // nonce_data_availability_mode
                    Felt252::zero().into(), // fee_data_availability_mode
                    Felt252::zero().into(), // Empty list of account_deployment_data
                    Felt252::zero().into(),
                ]);
            }
        };
