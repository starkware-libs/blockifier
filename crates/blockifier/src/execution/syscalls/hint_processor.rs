...
impl<'a> SyscallHintProcessor<'a> {
    ...
    pub fn execute_next_syscall(
        ...
        match selector {
            SyscallSelector::CallContract => {
                self.execute_syscall(vm, call_contract, self.context.versioned_constants().os_constants.gas_costs.call_contract_gas_cost)
            }
            SyscallSelector::Deploy => {
                self.execute_syscall(vm, deploy, self.context.versioned_constants().os_constants.gas_costs.deploy_gas_cost)
            }
            SyscallSelector::EmitEvent => {
                self.execute_syscall(vm, emit_event, self.context.versioned_constants().os_constants.gas_costs.emit_event_gas_cost)
            }
            SyscallSelector::GetBlockHash => {
                self.execute_syscall(vm, get_block_hash, self.context.versioned_constants().os_constants.gas_costs.get_block_hash_gas_cost)
            }
            SyscallSelector::GetExecutionInfo => self.execute_syscall(
                vm,
                get_execution_info,
                self.context.versioned_constants().os_constants.gas_costs.get_execution_info_gas_cost,
            ),
            SyscallSelector::Keccak => {
                self.execute_syscall(vm, keccak, self.context.versioned_constants().os_constants.gas_costs.keccak_gas_cost)
            }
            SyscallSelector::LibraryCall => {
                self.execute_syscall(vm, library_call, self.context.versioned_constants().os_constants.gas_costs.library_call_gas_cost)
            }
            SyscallSelector::LibraryCallL1Handler => self.execute_syscall(
                vm,
                library_call_l1_handler,
                self.context.versioned_constants().os_constants.gas_costs.library_call_gas_cost,
            ),
            SyscallSelector::ReplaceClass => {
                self.execute_syscall(vm, replace_class, self.context.versioned_constants().os_constants.gas_costs.replace_class_gas_cost)
            }
            SyscallSelector::Secp256k1Add => {
                self.execute_syscall(vm, secp256k1_add, self.context.versioned_constants().os_constants.gas_costs.secp256k1_add_gas_cost)
            }
            SyscallSelector::Secp256k1GetPointFromX => self.execute_syscall(
                vm,
                secp256k1_get_point_from_x,
                self.context.versioned_constants().os_constants.gas_costs.secp256k1_get_point_from_x_gas_cost,
            ),
            SyscallSelector::Secp256k1GetXy => self.execute_syscall(
                vm,
                secp256k1_get_xy,
                self.context.versioned_constants().os_constants.gas_costs.secp256k1_get_xy_gas_cost,
            ),
            SyscallSelector::Secp256k1Mul => {
                self.execute_syscall(vm, secp256k1_mul, self.context.versioned_constants().os_constants.gas_costs.secp256k1_mul_gas_cost)
            }
            SyscallSelector::Secp256k1New => {
                self.execute_syscall(vm, secp256k1_new, self.context.versioned_constants().os_constants.gas_costs.secp256k1_new_gas_cost)
            }
            SyscallSelector::Secp256r1Add => {
                self.execute_syscall(vm, secp256r1_add, self.context.versioned_constants().os_constants.gas_costs.secp256r1_add_gas_cost)
            }
            SyscallSelector::Secp256r1GetPointFromX => self.execute_syscall(
                vm,
                secp256r1_get_point_from_x,
                self.context.versioned_constants().os_constants.gas_costs.secp256r1_get_point_from_x_gas_cost,
            ),
            SyscallSelector::Secp256r1GetXy => self.execute_syscall(
                vm,
                secp256r1_get_xy,
                self.context.versioned_constants().os_constants.gas_costs.secp256r1_get_xy_gas_cost,
            ),
            SyscallSelector::Secp256r1Mul => {
                self.execute_syscall(vm, secp256r1_mul, self.context.versioned_constants().os_constants.gas_costs.secp256r1_mul_gas_cost)
            }
            SyscallSelector::Secp256r1New => {
                self.execute_syscall(vm, secp256r1_new, self.context.versioned_constants().os_constants.gas_costs.secp256r1_new_gas_cost)
            }
            SyscallSelector::SendMessageToL1 => self.execute_syscall(
                vm,
                send_message_to_l1,
                self.context.versioned_constants().os_constants.gas_costs.send_message_to_l1_gas_cost,
            ),
            SyscallSelector::StorageRead => {
                self.execute_syscall(vm, storage_read, self.context.versioned_constants().os_constants.gas_costs.storage_read_gas_cost)
            }
            SyscallSelector::StorageWrite => {
                self.execute_syscall(vm, storage_write, self.context.versioned_constants().os_constants.gas_costs.storage_write_gas_cost)
            }
        ...
        ) -> SyscallResult<Response>,
    {
        ...
        let required_gas = syscall_gas_cost - self.context.versioned_constants().os_constants.gas_costs.syscall_base_gas_cost;
