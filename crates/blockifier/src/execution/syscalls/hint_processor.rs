    ...
    fn allocate_tx_info_segment(&mut self, vm: &mut VirtualMachine) -> SyscallResult<Relocatable> {
        ...
        match account_tx_context {
            ...
            AccountTransactionContext::Deprecated(_) => {
                ...
                tx_info.extend_from_slice(&[
                    zero_felt.clone(), // Empty list of ResourceBound
                    zero_felt.clone(),
                    zero_felt.clone(), // Tip
                    zero_felt.clone(), // Empty list of paymaster_data
                    zero_felt.clone(),
                    zero_felt.clone(), // nonce_data_availability_mode
                    zero_felt.clone(), // fee_data_availability_mode
                    zero_felt.clone(), // Empty list of account_deployment_data
                    zero_felt,
                ]);
            }
        };
