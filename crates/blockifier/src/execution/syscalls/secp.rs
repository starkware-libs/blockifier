...
type Secp256k1GetPointFromXResponse = SecpOptionalEcPointResponse;
...
impl SyscallRequest for EcPointCoordinates {
    fn read(vm: &VirtualMachine, ptr: &mut Relocatable) -> SyscallResult<EcPointCoordinates> {
        let x = u256_from_ptr(vm, ptr)?;
        let y = u256_from_ptr(vm, ptr)?;
        Ok(EcPointCoordinates { x, y })
    }
}

type Secp256k1NewResponse = SecpOptionalEcPointResponse;
...
pub fn secp256r1_new(
    ...
    let modulos = <secp256r1::Fq as ark_ff::PrimeField>::MODULUS.into();
    let (x, y) = (request.x, request.y);
    if x >= modulos || y >= modulos {
        return Err(SyscallExecutionError::SyscallError {
            error_data: vec![
    ...
    let optional_ec_point_id =
        if ec_point.is_on_curve() && ec_point.is_in_correct_subgroup_assuming_on_curve() {
            Some(syscall_handler.allocate_secp256r1_point(ec_point))
        } else {
            None
        };
    Ok(Secp256r1NewResponse { optional_ec_point_id })
}
