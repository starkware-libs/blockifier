...
// STORED_BLOCK_HASH_BUFFER=10 blocks before the current block. Otherwise, returns an error.
pub fn get_block_hash(
    ...
    // We assure the requested block number is at least STORED_BLOCK_HASH_BUFFER blocks before the
    // current block.
    // We use checked_sub to avoid underflow when current_block_number < STORED_BLOCK_HASH_BUFFER.
