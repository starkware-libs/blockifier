...
// STORED_BLOCK_HASH_BUFFER blocks before the current block. Otherwise, returns an error.
    ...
    if upper_bound_block_number.is_none()
        || requested_block_number
            > upper_bound_block_number
                .expect("Underflow occurred while computing upper_bound_block_number.")
    {
        let out_of_gas_error = StarkFelt::try_from(BLOCK_NUMBER_OUT_OF_RANGE_ERROR)
