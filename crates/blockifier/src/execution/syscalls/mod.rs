...
impl SyscallRequest for GetBlockHashRequest {
    fn read(vm: &VirtualMachine, ptr: &mut Relocatable) -> SyscallResult<GetBlockHashRequest> {
        ...
        match felt.to_u64() {
            Some(value) => Ok(GetBlockHashRequest { block_number: BlockNumber(value) }),
            None => Err(SyscallExecutionError::InvalidSyscallInput {
                input: felt_to_stark_felt(&felt),
                info: "Should be a u64, representing a block number.".to_string(),
            }),
        }
    }
}
