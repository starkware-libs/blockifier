...
fn declare_tx_for_test(
    sender: FeatureContract,
    declared_contract: FeatureContract,
    version: TransactionVersion,
) -> (Transaction, BouncerInfo) {
    let zero_bounds = true;
    let declare_tx = Transaction::AccountTransaction(declare_tx(
        declare_tx_args! {
            sender_address: sender.get_instance_address(0),
    ...
    (declare_tx, expected_bouncer_info)
}
...
#[case::declare_tx(TransactionType::Declare, TransactionVersion::THREE)]
...
fn test_tx_executor(
    ...
    let (tx, expected_bouncer_info) = match tx_type {
        TransactionType::Declare => declare_tx_for_test(account, empty, version),
