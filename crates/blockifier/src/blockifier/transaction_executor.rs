...
impl<S: StateReader + Send + Sync> TransactionExecutor<S> {
    ...
    pub fn execute_chunk(
        ...
        let worker_executor = Arc::new(WorkerExecutor::new(
            chunk_state.clone(),
            chunk,
        ...
        // consuming it. Drop the worker executor that holds a reference to the versioned state so
        // we could consume it.
        drop(worker_executor);
        let mut versioned_state_after_use = chunk_state.into_inner_state();
        versioned_state_after_use.commit(committed_chunk_size);

        let block_state_after_use = versioned_state_after_use.into_initial_state();
