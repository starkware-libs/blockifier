...
impl GasCosts {
    // List of additinal fields, beside the fields of GasCosts, that can appear in the JSON. These
    // fields will be ignored during the creation of the GasCosts object.
    const ADDITIONAL_ALLOWED_NAMES: [&'static str; 25] = [
...
impl TryFrom<&OsConstantsRawJson> for GasCosts {
    ...
    fn try_from(raw_json_data: &OsConstantsRawJson) -> Result<Self, Self::Error> {
        let gas_costs: GasCosts = serde_json::to_value(&raw_json_data.parse_gas_costs()?)
...
impl OsConstantsRawJson {
    fn parse_gas_costs(&self) -> Result<IndexMap<String, u64>, OsConstantsSerdeError> {
        ...
        let allowed_additional_names: IndexSet<_> =
            GasCosts::ADDITIONAL_ALLOWED_NAMES.iter().copied().collect();
        for (key, value) in &self.raw_json_file_as_dict {
            if allowed_additional_names.contains(key.as_str()) {
                // Ignore additinal consts.
                continue;
            }
...
pub enum VersionedConstantsError {
    ...
    MissingOSConstants,
    #[error(transparent)]
    InvalidJSONObject(#[from] utils::UtilsError),
}
