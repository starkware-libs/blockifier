...
pub struct ResourceCost {
    pub numerator: u128,
    pub denominator: NonZeroU128,
}

impl ResourceCost {
    pub fn new(numerator: u128, denominator: NonZeroU128) -> Self {
        Self { numerator, denominator }
    }

    pub fn one() -> Self {
        Self { numerator: 1, denominator: NonZeroU128::new(1).expect("1 != 0.") }
    }

    pub fn floor_cost(&self, amount: u128) -> u128 {
        (amount * self.numerator) / self.denominator
    }

    pub fn ceil_cost(&self, amount: u128) -> u128 {
        // Due to error E0658, do not use div_ceil.
        let new_numerator = amount * self.numerator;
        let div_floor = new_numerator / self.denominator.get();
        match new_numerator % self.denominator.get() {
            0 => div_floor,
            _ => div_floor + 1,
        }
    }

    pub fn fraction(&self) -> (u128, NonZeroU128) {
...
impl VersionedConstants {
    ...
    pub fn create_for_account_testing() -> Self {
        ...
        let vm_resource_fee_cost = Arc::new(HashMap::from([
            (crate::abi::constants::N_STEPS_RESOURCE.to_string(), ResourceCost::one()),
            (HASH_BUILTIN_NAME.to_string(), ResourceCost::one()),
            (RANGE_CHECK_BUILTIN_NAME.to_string(), ResourceCost::one()),
            (SIGNATURE_BUILTIN_NAME.to_string(), ResourceCost::one()),
            (BITWISE_BUILTIN_NAME.to_string(), ResourceCost::one()),
            (POSEIDON_BUILTIN_NAME.to_string(), ResourceCost::one()),
            (OUTPUT_BUILTIN_NAME.to_string(), ResourceCost::one()),
            (EC_OP_BUILTIN_NAME.to_string(), ResourceCost::one()),
        ]));
