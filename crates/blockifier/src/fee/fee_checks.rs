...
use crate::transaction::objects::{
    FeeType, GasAndBlobGasUsages, HasRelatedFeeType, TransactionExecutionResult, TransactionInfo,
};
        ...
        let recommended_fee = match error {
            ...
            FeeCheckError::MaxFeeExceeded { .. } | FeeCheckError::MaxL1GasAmountExceeded { .. } => {
                match &tx_context.tx_info {
                    TransactionInfo::Current(context) => get_fee_by_l1_gas_usage(
        ...
        match tx_info {
            TransactionInfo::Current(context) => {
                ...
                let GasAndBlobGasUsages { gas_usage, blob_gas_usage } =
                    calculate_tx_l1_gas_usages(actual_resources, versioned_constants)?;
