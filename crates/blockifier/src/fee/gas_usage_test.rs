...
fn test_get_estimated_strk_price() {
    let (wei_1, wei_2, wei_3) = (BigUint::from(10_u32), BigUint::from(14_u32), BigUint::from(12_u32));
    let (strk_1, strk_2, strk_3) = (BigUint::from(50_u32), BigUint::from(42_u32), BigUint::from(24_u32));
    ...
    // let state_4 = PoolState { total_wei: wei_1, total_strk: strk_1 + BigUint::from(10_u32) };
    ...
    assert_eq!(
        PoolStateAggregator::new(&vec![state_1.clone()]).unwrap().convert_wei_to_strk(wei_amount.clone()),
    ...
    assert_eq!(
        PoolStateAggregator::new(&vec![state_3.clone(), state_1.clone()]).unwrap().convert_wei_to_strk(wei_amount.clone()),
        (state_3.total_strk.clone() / state_3.total_wei.clone()) * wei_amount.clone()
    );
    assert_eq!(
        PoolStateAggregator::new(&vec![state_3, state_1.clone(), state_2.clone()]).unwrap().convert_wei_to_strk(wei_amount.clone()),
    ...
    // assert_eq!(
    //     PoolStateAggregator::new(&vec![state_1.clone(), state_4.clone()]).unwrap().convert_wei_to_strk(wei_amount.clone()),
