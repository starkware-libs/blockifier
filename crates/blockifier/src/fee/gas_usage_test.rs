...
use crate::fee::gas_usage::{
    calculate_tx_blob_gas_usage, calculate_tx_gas_usage_for_da, calculate_tx_gas_usage_without_da,
    get_consumed_message_to_l2_emissions_cost, get_log_message_to_l1_emissions_cost,
    get_message_segment_length, get_onchain_data_cost, get_onchain_data_segment_length,
};
...
fn test_calculate_tx_gas_usage_basic() {
    ...
    let deploy_account_gas_usage =
        calculate_tx_gas_usage_for_da(deploy_account_state_changes_count).unwrap();
    ...
    let l1_handler_gas_usage =
        calculate_tx_gas_usage_without_da(std::iter::empty(), Some(l1_handler_payload_size))
            .unwrap();
    ...
    let l2_to_l1_messages_gas_usage =
        calculate_tx_gas_usage_without_da(call_infos_iter.clone(), None).unwrap()
            + calculate_tx_gas_usage_for_da(l2_to_l1_state_changes_count).unwrap();
    ...
    let storage_writings_gas_usage =
        calculate_tx_gas_usage_for_da(storage_writes_state_changes_count).unwrap();
    ...
    let gas_usage =
        calculate_tx_gas_usage_without_da(call_infos_iter, Some(l1_handler_payload_size)).unwrap()
            + calculate_tx_gas_usage_for_da(combined_state_changes_count).unwrap();
