...
fn test_calculate_tx_gas_usage_basic(#[values(false, true)] use_kzg_da: bool) {
    ...
    assert_eq!(
        u128::try_from(manual_starknet_gas_usage + manual_sharp_gas_usage).unwrap(),
        deploy_account_gas_usage,
    );
    assert_eq!(u128::try_from(manual_sharp_blob_gas_usage).unwrap(), deploy_account_blob_gas_usage);
    ...
    assert_eq!(
        u128::try_from(manual_sharp_blob_gas_usage).unwrap(),
    ...
    assert_eq!(
        u128::try_from(manual_starknet_blob_gas_usage + manual_sharp_blob_gas_usage).unwrap(),
    ...
    let expected_gas_usage = l1_handler_gas_usage
        ...
        + u128::try_from(fee_balance_discount).unwrap();
...
fn test_onchain_data_discount() {
    ...
    let cost_ratio =
        usize_into_f64(actual_cost).unwrap() / usize_into_f64(cost_without_discount).unwrap();
