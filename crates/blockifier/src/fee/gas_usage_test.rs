...
fn test_calculate_tx_gas_usage_basic() {
    ...
    let expected_gas_usage = l1_handler_gas_usage.unwrap()
        + l2_to_l1_messages_gas_usage.unwrap()
        + storage_writings_gas_usage.unwrap()
        + fee_balance_discount;

    assert_eq!(gas_usage.unwrap(), expected_gas_usage);
}
