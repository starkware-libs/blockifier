...
fn test_calculate_tx_gas_usage_basic(#[values(false, true)] use_kzg_da: bool) {
    ...
    assert_eq!(
        u128_from_usize(manual_starknet_gas_usage + manual_sharp_gas_usage),
        deploy_account_gas_usage,
    );
    assert_eq!(u128_from_usize(manual_sharp_blob_gas_usage), deploy_account_blob_gas_usage);
    ...
    assert_eq!(u128_from_usize(manual_sharp_blob_gas_usage), l2_to_l1_messages_blob_gas_usage);
    ...
    assert_eq!(
        u128_from_usize(manual_starknet_blob_gas_usage + manual_sharp_blob_gas_usage),
    ...
    .unwrap();
    let GasAndBlobGasUsages { gas_usage, blob_gas_usage } = gas_usage_and_blob_gas_usage;
    ...
    assert_eq!(blob_gas_usage, expected_blob_gas_usage);
}
