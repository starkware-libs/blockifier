...
use crate::fee::gas_usage::{
    ...
    calculate_tx_gas_usage_messages_and_proof, get_consumed_message_to_l2_emissions_cost,
...
fn test_calculate_tx_gas_usage_basic() {
    ...
    let l1_handler_gas_usage = calculate_tx_gas_usage_messages_and_proof(
        std::iter::empty(),
    ...
    let l2_to_l1_messages_gas_usage =
        calculate_tx_gas_usage_messages_and_proof(call_infos_iter.clone(), None).unwrap()
    ...
    let gas_usage =
        calculate_tx_gas_usage_messages_and_proof(call_infos_iter, Some(l1_handler_payload_size))
            .unwrap()
