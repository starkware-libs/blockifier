...
pub fn get_onchain_data_segment_length(
    ...
    onchain_data_segment_length += n_class_updates * abi_constants::CLASS_UPDATE_SIZE;
...
pub fn get_message_segment_length(
    ...
    let mut message_segment_length = l2_to_l1_payloads_length
        .iter()
        .map(|payload_length| abi_constants::L2_TO_L1_MSG_HEADER_SIZE + payload_length)
        .sum();
    ...
    if let Some(payload_size) = l1_handler_payload_size {
        ...
        message_segment_length += abi_constants::L1_TO_L2_MSG_HEADER_SIZE + payload_size;
    }
    ...
    match l1_handler_payload_size {
        ...
        Some(l1_handler_payload_size) => {
            get_event_emission_cost(
                abi_constants::CONSUMED_MSG_TO_L2_N_TOPICS,
                // We're assuming the existence of one (not indexed) payload array.
                abi_constants::CONSUMED_MSG_TO_L2_ENCODED_DATA_SIZE + l1_handler_payload_size,
            )
        }
    }
}
...
pub fn get_log_message_to_l1_emissions_cost(l2_to_l1_payloads_length: &[usize]) -> usize {
    l2_to_l1_payloads_length
        ...
        .map(|length| {
            get_event_emission_cost(
                abi_constants::LOG_MSG_TO_L1_N_TOPICS,
                // We're assuming the existence of one (not indexed) payload array.
                abi_constants::LOG_MSG_TO_L1_ENCODED_DATA_SIZE + *length,
            )
        })
        .sum()
}
...
fn get_event_emission_cost(n_topics: usize, data_length: usize) -> usize {
    eth_gas_constants::GAS_PER_LOG
        + (n_topics + abi_constants::N_DEFAULT_TOPICS) * eth_gas_constants::GAS_PER_LOG_TOPIC
