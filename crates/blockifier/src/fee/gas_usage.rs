...
pub fn estimate_minimal_fee(
    ...
    // Nonce increment + fee transfer change two storage cells on one contract.
    let resources = ResourcesMapping(HashMap::from([
        (constants::GAS_USAGE.to_string(), get_onchain_data_segment_length(1, 2, 0)),
        (
            constants::N_STEPS_RESOURCE.to_string(),
            OS_RESOURCES.execute_txs_inner().get(&tx.tx_type()).unwrap().n_steps,
        ),
    ]));
