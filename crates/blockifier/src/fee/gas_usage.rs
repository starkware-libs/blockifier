...
// Return the gas cost for transaction calldata.
pub fn get_calldata_gas_cost(calldata_length: usize) -> GasVector {
    GasVector {
        l1_gas: u128_from_usize(calldata_length * eth_gas_constants::GAS_PER_CALLDATA_BYTE)
            .expect("Could not convert calldata bytes gas usage from usize to u128."),
        blob_gas: 0,
    }
}
