...
pub fn get_onchain_data_cost(state_changes_count: StateChangesCount) -> usize {
    ...
    let mut naive_cost = onchain_data_segment_length * eth_gas_constants::SHARP_GAS_PER_MEMORY_WORD;
    ...
    naive_cost -= state_changes_count.n_modified_contracts * nonce_felt_discount;

    // Up to balance of 8**10 ETH.
    let fee_balance_value_cost = eth_gas_constants::get_calldata_word_cost(12);
    let fee_balance_value_discount =
        eth_gas_constants::GAS_PER_MEMORY_WORD - fee_balance_value_cost;

    if naive_cost < fee_balance_value_discount {
        // Cost must be non-negative after discount.
        naive_cost
    } else {
        naive_cost - fee_balance_value_discount
    }
}
