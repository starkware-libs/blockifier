...
pub fn get_events_milligas_cost(
    ...
    events
        ...
        .map(|OrderedEvent { event, .. }| {
            ...
            // num_bytes_data.
            let num_keys = safe_u128_from_usize(event.keys.len());
            let num_data_words = safe_u128_from_usize(event.data.0.len());
            event_key_factor * data_word_cost * num_keys + data_word_cost * num_data_words
        })
        .sum()
}
...
pub fn get_calldata_and_signature_gas_cost(
    ...
    let l2_resource_gas_costs = versioned_constants.get_l2_resource_gas_costs();
    let n_data_words = u128_from_usize(calldata_length + signature_length)
        .expect("Could not convert n_data_words from usize to u128");
    let calldata_and_signature_gas_cost =
        n_data_words * l2_resource_gas_costs.milligas_per_data_felt / 1000;

    GasVector { l1_gas: calldata_and_signature_gas_cost, l1_data_gas: 0 }
}
...
pub fn get_code_gas_cost(
    ...
    if let Some(class_info) = class_info {
        ...
        let code_gas_cost = total_code_size * l2_resource_gas_costs.milligas_per_code_byte / 1000;
        GasVector { l1_gas: code_gas_cost, l1_data_gas: 0 }
    } else {
