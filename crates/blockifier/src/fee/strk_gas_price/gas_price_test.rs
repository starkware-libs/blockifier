...
#[test]
fn test_convert_wei_to_fri() {
    let (wei_1, wei_2, wei_3) =
        (BigUint::from(10_u32), BigUint::from(14_u32), BigUint::from(12_u32));
    let (fri_1, fri_2, fri_3, fri_4) = (
        BigUint::from(50_u32),
        BigUint::from(42_u32),
        BigUint::from(24_u32),
        BigUint::from(150_u32),
    );
    let state_1 = PoolState { total_wei: wei_1.clone(), total_fri: fri_1 };
    let state_2 = PoolState { total_wei: wei_2, total_fri: fri_2 };
    let state_3 = PoolState { total_wei: wei_3, total_fri: fri_3 };
    let state_4 = PoolState { total_wei: wei_1, total_fri: fri_4 };
    ...
    assert_eq!(
        PoolStateAggregator::new(&[state_1.clone()])
            .unwrap()
            .convert_wei_to_fri(wei_amount.clone()),
        (state_1.total_fri.clone() * wei_amount.clone()) / state_1.total_wei.clone()
    );
    ...
    assert_eq!(
        PoolStateAggregator::new(&[state_3.clone(), state_1.clone()])
            .unwrap()
            .convert_wei_to_fri(wei_amount.clone()),
        (state_3.total_fri.clone() * wei_amount.clone()) / state_3.total_wei.clone()
    );
    assert_eq!(
        PoolStateAggregator::new(&[state_3, state_1.clone(), state_2.clone()])
            .unwrap()
            .convert_wei_to_fri(wei_amount.clone()),
        (state_2.total_fri.clone() * wei_amount.clone()) / state_2.total_wei.clone()
    );
    ...
    assert_eq!(
        PoolStateAggregator::new(&[state_1.clone(), state_4.clone()])
            .unwrap()
            .convert_wei_to_fri(wei_amount.clone()),
        ((state_1.total_fri + state_4.total_fri) * wei_amount)
