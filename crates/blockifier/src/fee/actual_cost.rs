...
use super::gas_usage::get_onchain_data_segment_length;
...
impl<'a> ActualCostBuilder<'a> {
    ...
    fn calculate_actual_fee_and_resources(
        ...
        let data_segment_length = get_onchain_data_segment_length(
            &self.tx_resources.starknet_resources.state_changes_count,
        );
        let total_vm_usage = execution_resources
            + &self.tx_context.block_context.versioned_constants.get_additional_os_tx_resources(
                self.tx_type,
                self.tx_resources.starknet_resources.calldata_length,
                data_segment_length,
                use_kzg_da,
            )?;
        self.tx_resources.vm_resources = total_vm_usage.filter_unused_builtins();

        // TODO(Nimrod, 25/3/2024): What should be the type of bouncer resources?
        // (TransactionResources??, which is different only at n_steps?) resources
