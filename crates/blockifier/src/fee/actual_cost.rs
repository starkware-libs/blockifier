...
impl<'a> ActualCostBuilder<'a> {
    ...
    /// given the number of reverted steps.
    /// If execution succeeded, but the transaction must be reverted for other reasons, the
    /// precomputed (post-execute) fee should be provided.
    ...
    fn calculate_actual_fee_and_resources(
        ...
        *actual_resources.0.get_mut(&abi_constants::N_STEPS_RESOURCE.to_string()).unwrap() +=
        ...
        // Non-revertible transactions may fail on fee transfer - which should happen if the actual
        // fee is greater than the max fee.
