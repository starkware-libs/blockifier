...
fn test_calculate_tx_gas_usage_basic(#[values(false, true)] use_kzg_da: bool) {
    ...
    for cairo_version in [CairoVersion::Cairo0, CairoVersion::Cairo1] {
        ...
        let code_milligas_cost = versioned_constants.l2_resource_gas_costs.gas_per_code_byte
            * u128_from_usize(
        ...
        let manual_gas_vector =
            GasVector { l1_gas: code_milligas_cost.to_integer(), ..Default::default() };
    ...
    let manual_starknet_gas_usage = calldata_and_signature_milligas_cost.to_integer();
    ...
    let manual_starknet_gas_usage = message_segment_length * eth_gas_constants::GAS_PER_MEMORY_WORD
        ...
        .unwrap()
        + usize_from_u128(calldata_and_signature_milligas_cost.to_integer()).unwrap();
