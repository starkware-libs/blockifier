...
pub fn calculate_l1_gas_by_vm_usage(
    ...
    // TODO(Nimrod, 1/5/2024): We don't remove `SEGMENT_ARENA_BUILTIN_NAME` from VM resources when
    // we calculate total_n_steps. Should we do that? vm_resource_fee_costs doesn't contain it.
    ...
    let vm_builtin_gas_usage = vm_resource_fee_costs
        .iter()
    ...
    Ok(GasVector::from_l1_gas(vm_builtin_gas_usage))
}
