...
impl BlockNumberAndHash {
    pub fn new(block_number: BlockNumber, block_hash: BlockHash) -> BlockNumberAndHash {
        BlockNumberAndHash { block_number, block_hash }
    }
}
...
pub fn pre_process_block(
    ...
    // check of old_block_number_and_hash is None
    match old_block_number_and_hash {
        None => {
            if block_context_args.block_number >= BlockNumber(STORED_BLOCK_HASH_BUFFER) {
