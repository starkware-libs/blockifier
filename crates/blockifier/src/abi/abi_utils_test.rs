...
fn test_selector_from_name() {
    ...
    let expected_default_selector =
        EntryPointSelector(Felt::from_hex_unchecked(abi_constants::DEFAULT_ENTRY_POINT_SELECTOR));
    assert_eq!(
    ...
    let expected_execute_selector =
        EntryPointSelector(Felt::from_hex(expected_execute_selector).unwrap());
    assert_eq!(
    ...
    let expected_empty_selector =
        EntryPointSelector(Felt::from_hex(expected_empty_selector).unwrap());
