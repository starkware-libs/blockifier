...
use crate::fee::os_usage::{
    get_additional_tx_type_os_resources, get_entry_point_syscall_resources,
};
...
pub fn calculate_tx_resources(
    ...
    let vm_and_syscall_usage = &execution_resources.vm_resources
        + &get_entry_point_syscall_resources(&execution_resources.syscall_counter)?;
    let total_vm_usage =
        &vm_and_syscall_usage + &get_additional_tx_type_os_resources(tx_type, calldata_length)?;
