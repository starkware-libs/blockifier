...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    // Thus, the actual consumed resources may be greater than the bounds in the `charge_fee=false`
    // case (and may be greater than the upper bounds if `charge_fee=true` as well, since the
    // execution step limit derived from the resource bounds is not tight).
