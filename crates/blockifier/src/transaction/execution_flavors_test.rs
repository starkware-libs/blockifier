...
fn l(
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(
        u64_from_usize(create_one_syscall(DeprecatedSyscallSelector::CallContract).n_steps)
            + u64_from_usize(create_one_transaction(TransactionType::InvokeFunction).n_steps)
            + 1738,
        validate,
        &fee_type,
    );
...
fn test_simulate_validate_charge_fee_fail_validate(
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(
        u64_from_usize(create_one_transaction(TransactionType::InvokeFunction).n_steps) + 27229,
        validate,
        &fee_type,
    );
    let result = account_invoke_tx(invoke_tx_args! {
...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    let (revert_gas_used, revert_fee) = gas_and_fee(
        u64_from_usize(create_one_transaction(TransactionType::InvokeFunction).n_steps) + 1719,
        validate,
        &fee_type,
    );
...
fn test_simulate_validate_charge_fee_post_execution(
    ...
    let (revert_gas_usage, revert_fee) = gas_and_fee(
        u64_from_usize(create_one_transaction(TransactionType::InvokeFunction).n_steps) + 5730,
        validate,
        &fee_type,
    );
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(
    ...
    let (fail_actual_gas, fail_actual_cost) = gas_and_fee(
        u64_from_usize(create_one_transaction(TransactionType::InvokeFunction).n_steps + 2252),
        validate,
        &fee_type,
    );
