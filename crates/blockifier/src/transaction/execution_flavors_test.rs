...
fn test_simulate_validate_charge_fee_pre_validate(
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(6057, validate, &fee_type);
...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    let (revert_gas_used, revert_fee) = gas_and_fee(5278, validate, &fee_type);
    ...
    let (limited_gas_used, limited_fee) = gas_and_fee(7658, validate, &fee_type);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10049, validate, &fee_type);
    let tx_execution_info = account_invoke_tx(invoke_tx_args! {
        max_fee: fee_bound,
    ...
    let block_limit_gas = invoke_tx_max_n_steps_as_u64 + 1657;
...
fn test_simulate_validate_charge_fee_post_execution(
    ...
    // than the gas bound.
    let (revert_gas_usage, revert_fee) = gas_and_fee(9289, validate, &fee_type);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10049, validate, &fee_type);
    let tx_execution_info = account_invoke_tx(invoke_tx_args! {
    ...
    let (success_actual_gas, actual_fee) = gas_and_fee(8567, validate, &fee_type);
    let (fail_actual_gas, fail_actual_cost) = gas_and_fee(5817, validate, &fee_type);
