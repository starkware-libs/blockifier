...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    // Thus, the actual consumed resources may be greater in the `charge_fee=false` case (and may be
    // greater than the upper bounds regardless of the value of `charge_fee`, as the execution step
    // limit derived from the resource bounds is not tight).
...
fn test_simulate_validate_charge_fee_post_execution(
    ...
    let (success_actual_gas, actual_fee) = gas_and_fee(13923, validate);
    let (fail_actual_gas, _) = gas_and_fee(6880, validate);
