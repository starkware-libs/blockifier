...
fn test_simulate_validate_charge_fee_pre_validate(
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(6135, validate, &fee_type);
...
fn test_simulate_validate_charge_fee_fail_validate(
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(30860, validate, &fee_type);
    let result = account_invoke_tx(invoke_tx_args! {
        max_fee,
        resource_bounds: l1_resource_bounds(MAX_L1_GAS_AMOUNT, MAX_L1_GAS_PRICE),
...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    let (revert_gas_used, revert_fee) = gas_and_fee(5352, validate, &fee_type);
    ...
    let (limited_gas_used, limited_fee) = gas_and_fee(7729, validate, &fee_type);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10127, validate, &fee_type);
    let tx_execution_info = account_invoke_tx(invoke_tx_args! {
        max_fee: fee_bound,
    ...
    let block_limit_gas = invoke_tx_max_n_steps_as_u64 + 1728;
...
fn test_simulate_validate_charge_fee_post_execution(
    ...
    // than the gas bound.
    let (revert_gas_usage, revert_fee) = gas_and_fee(9363, validate, &fee_type);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10127, validate, &fee_type);
    let tx_execution_info = account_invoke_tx(invoke_tx_args! {
    ...
    let (success_actual_gas, actual_fee) = gas_and_fee(8645, validate, &fee_type);
    let (fail_actual_gas, fail_actual_cost) = gas_and_fee(5893, validate, &fee_type);
