...
impl ExecuteTransaction for DeclareTransaction {
    ...
    // Calldata for validation contains only the hash of the declared class (a parameter of the
    // transaction that cannot be obtained using `get_tx_info()`).
...
impl ExecuteTransaction for DeployAccountTransaction {
    ...
    /// Calldata for validation contains class_hash, salt and constructor_calldata (parameters of
    /// the transaction that cannot be obtained using `get_tx_info()`).
    fn validate_entrypoint_calldata(&self) -> Calldata {
...
impl ExecuteTransaction for InvokeTransaction {
    ...
    // Calldata for validation is the same calldata as for·
