...
impl DeclareTransaction {
    ...
    pub fn get_account_tx_context(&self) -> AccountTransactionContext {
        if self.tx.version() == TransactionVersion::ZERO
            || self.tx.version() == TransactionVersion::ONE
            || self.tx.version() == TransactionVersion::TWO
        {
            return AccountTransactionContext::Deprecated(
                DeprecatedAccountTransactionContext::new(
                    self.tx_hash(),
                    self.max_fee(),
                    self.tx.version(),
                    self.tx.signature(),
        ...
        match &self.tx {
            starknet_api::transaction::DeclareTransaction::V3(tx) => {
                AccountTransactionContext::Current(CurrentAccountTransactionContext::new(
                    self.tx_hash(),
                    self.max_fee(),
                    self.tx.version(),
                    self.tx.signature(),
...
impl DeployAccountTransaction {
    ...
    pub fn get_account_tx_context(&self) -> AccountTransactionContext {
        if self.tx.version() == TransactionVersion::ONE {
            return AccountTransactionContext::Deprecated(
                DeprecatedAccountTransactionContext::new(
                    self.tx_hash,
                    self.max_fee(),
