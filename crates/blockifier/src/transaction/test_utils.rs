...
pub fn create_account_tx_for_validate_test(
    ...
    match tx_type {
        TransactionType::Declare => declare_tx(
            declare_tx_args! {
                class_hash: FeatureContract::TestContract(CairoVersion::Cairo0).get_class_hash(),
                sender_address,
                signature,
            },
        ...
        TransactionType::DeployAccount => {
            // The sender_address is not used here because the real sender address is by running the
            // transaction.
