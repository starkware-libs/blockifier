...
pub fn create_account_tx_for_validate_test(
    ...
    match tx_type {
        TransactionType::Declare => {
            ...
            let class_info =
                calculate_class_info_for_testing(declared_contract.get_class()).unwrap();
            declare_tx(
                declare_tx_args! {
                    class_hash,
                    sender_address,
...
) -> Result<ClassInfo, TransactionExecutionError> {
    let sierra_program_length = match contract_class {
        ContractClass::V0(_) => 0,
        ContractClass::V1(_) => 100,
    };
    ClassInfo::new(&contract_class, sierra_program_length, 100)
}
