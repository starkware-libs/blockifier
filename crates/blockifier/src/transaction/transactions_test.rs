...
fn test_validate_accounts_tx(
    ...
    if let CairoVersion::Cairo0 = cairo_version {
        // Calling the syscall get_block_number and asserting the returned blocknumber was modified
        // for validate.
        let account_tx = create_account_tx_for_validate_test(
            nonce_manager,
            FaultyAccountTxCreatorArgs {
        ...
        assert!(account_tx.execute(state, block_context, true, true).err().is_none());
        // Calling the syscall get_block_timestamp and asserting the returned timestamp was modified
        // for validate.
        let account_tx = create_account_tx_for_validate_test(
            nonce_manager,
            FaultyAccountTxCreatorArgs {
    ...
    if let CairoVersion::Cairo1 = cairo_version {
        let account_tx = create_account_tx_for_validate_test(
            // Calling the syscall get_execution_info and asserting the returned block_info was
