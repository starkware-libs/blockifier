...
fn expected_fee_transfer_call_info(
    ...
    let (sender_balance_key_low, sender_balance_key_high) = SierraU256::get_explicit_storage_keys(
        get_erc20_balances_var_address(&account_address).unwrap(),
    )
    .expect("Cannot get sender balance keys.");
    let (sequencer_balance_key_low, sequencer_balance_key_high) =
        SierraU256::get_explicit_storage_keys(
...
fn validate_final_balances(
    ...
    assert_eq!(
        state.get_fee_token_balance(block_context, &contract_address).unwrap().to_biguint(),
    ...
    assert_eq!(
        state
            .get_fee_token_balance(
                block_context,
                &ContractAddress::try_from(stark_felt!(TEST_SEQUENCER_ADDRESS)).unwrap()
            )
            .unwrap()
            .to_biguint(),
...
fn test_invoke_tx(
    ...
    validate_final_balances(
        ...
        ContractAddress::try_from(stark_felt!(TEST_ACCOUNT_CONTRACT_ADDRESS)).unwrap(),
...
fn test_state_get_fee_token_balance(state: &mut CachedState<DictStateReader>) {
    ...
    let balance = state
        .get_fee_token_balance(block_context, &contract_address!(recipient))
        .unwrap()
        .to_biguint();

    let minted = SierraU256 { low_val: mint_low, high_val: mint_high }.to_biguint();
    // TODO(barak, 01/10/2023): Implement PartialEq and Eq for SierraType and remove the conversion
    // to BigUint.
