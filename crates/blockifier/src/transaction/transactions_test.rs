...
fn create_state_account_without_validation() -> CachedState<DictStateReader> {
...
#[test]
fn test_invoke_tx() {
    let state = &mut create_state_account_without_validation();
    let block_context = &BlockContext::create_for_testing();
...
fn test_negative_invoke_tx_flows() {
    ...
    let execution_error = invalid_tx
        .execute(&mut create_state_account_without_validation(), block_context)
        .unwrap_err();
