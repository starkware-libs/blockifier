...
use crate::abi::abi_utils::{
    get_erc20_balance_var_address, get_storage_var_address, selector_from_name,
};
...
fn expected_fee_transfer_call_info(
    ...
    let (sender_balance_key_low, sender_balance_key_high) =
        SierraU256::get_storage_keys(get_erc20_balance_var_address(&account_address).unwrap())
            .expect("Cannot get sender balance keys.");
    let (sequencer_balance_key_low, sequencer_balance_key_high) = SierraU256::get_storage_keys(
        get_erc20_balance_var_address(&block_context.sequencer_address).unwrap(),
    )
