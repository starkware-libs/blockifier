...
fn test_invoke_tx(
    ...
    let expected_actual_fee =
        actual_execution_info.transaction_receipt.resources.calculate_tx_fee(block_context, fee_type).unwrap();
...
fn test_l1_handler(#[values(false, true)] use_kzg_da: bool) {
    ...
    let expected_actual_fee =
        (expected_execution_info.transaction_receipt.resources.calculate_tx_fee(block_context, &FeeType::Eth))
            .unwrap();
    assert_matches!(
        error,
