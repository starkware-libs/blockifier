...
fn valid_invoke_tx() -> InvokeTransaction {
...
fn test_negative_invoke_tx_flows() {
    let mut state = create_test_state();
    let block_context = BlockContext::create_for_testing();
    let valid_invoke_tx = valid_invoke_tx();
...
// Declare.
fn valid_declare_tx() -> DeclareTransaction {
    DeclareTransaction {
        max_fee: Fee(1),
...
fn test_declare_tx() {
    ...
    let declare_tx = valid_declare_tx();
...
// DeployAccount.
fn valid_deploy_account_tx() -> DeployAccountTransaction {
...
fn test_deploy_account_tx() {
    ...
    // the transaction.
    let deploy_account_tx = valid_deploy_account_tx();
