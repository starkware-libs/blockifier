...
fn test_invoke_tx() {
    ...
    // TODO(Noa, 01/05/23): Calculate the actual fee using the transaction.
    let expected_actual_fee = Fee(530600000000000);
...
fn test_declare_tx() {
    ...
    // Build expected fee transfer call info.
    // TODO(Noa, 01/05/23): Calculate the actual fee using the transaction.
    let expected_actual_fee = Fee(391200000000000);
    let expected_fee_transfer_call_info = expected_fee_transfer_call_info(
        block_context,
        expected_account_address,
        expected_actual_fee,
...
fn test_deploy_account_tx() {
    ...
    // Build expected fee transfer call info.
    // TODO(Noa, 01/05/23): Calculate the actual fee using the transaction.
    let expected_actual_fee = Fee(664400000000000);
    let expected_fee_transfer_call_info = expected_fee_transfer_call_info(
        block_context,
        deployed_account_address,
        expected_actual_fee,
