...
fn expected_fee_transfer_call_info(
    ...
    let sender_balance_key_low = get_erc20_balance_var_address(&account_address)
        .expect("Cannot get sender balance low key.");
    let sender_balance_key_high =
        next_storage_key(&sender_balance_key_low).expect("Cannot get sender balance high key.");
    let sequencer_balance_key_low = get_erc20_balance_var_address(&block_context.sequencer_address)
        .expect("Cannot get sequencer balance low key.");
...
fn validate_final_balances(
    ...
    assert_eq!(
        state
            .get_fee_token_balance(block_context, &contract_address!(TEST_SEQUENCER_ADDRESS))
            .unwrap()
            .to_biguint(),
