...
use crate::test_utils::contracts::FeatureContract as FC;
...
fn test_only_query_flag(#[case] only_query: bool) {
    ...
    let block_context = &BlockContext::create_for_account_testing();
    let state = &mut test_state(block_context, account_balance);
    ...
    let sender_address = FC::AccountWithoutValidations(CairoVersion::Cairo1).get_address(0);
    let test_contract_address = FC::TestContract(CairoVersion::Cairo1).get_address(0);
