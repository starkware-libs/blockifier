...
fn test_declare_tx(
    ...
    let expected_range_check_builtin =
        if cairo_version == CairoVersion::Cairo0 || version == TransactionVersion::ZERO {
            63
        } else {
            65
        };
    let expected_n_steps_resource = if version == TransactionVersion::ZERO {
        ...
        if cairo_version == CairoVersion::Cairo0 { 2809 } else { 2847 }
    };
    ...
    let account_tx = declare_tx(
        DeclareTxArgs {
            ...
            sender_address,
            max_fee: Fee(MAX_FEE),
            resource_bounds: l1_resource_bounds(MAX_L1_GAS_AMOUNT, MAX_L1_GAS_PRICE as u128),
            version,
            ..Default::default()
        },
    ...
    let expected_memory_words = match version {
        TransactionVersion::ZERO => 2,
        TransactionVersion::ONE => 4, // 1 modified contract, 1 storage update (sender balance).
        TransactionVersion::TWO | TransactionVersion::THREE => 6,
