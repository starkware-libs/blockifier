...
fn test_insufficient_resource_bounds(account_cairo_version: CairoVersion) {
    ...
    // TODO(Aner, 21/01/24) cahnge to linear combination
...
fn test_calculate_tx_gas_usage() {
    ...
    let (l1_gas_usage, _l1_blob_gas_usage): (usize, usize) = (
        l1_gas_and_blob_gas_usage.gas_usage.try_into().unwrap(),
        l1_gas_and_blob_gas_usage.blob_gas_usage.try_into().unwrap(),
    );
