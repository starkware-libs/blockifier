...
fn test_validate_accounts_tx(#[case] tx_type: TransactionType) {
    ...
    let faulty_account = FeatureContract::FaultyAccount(CairoVersion::Cairo0);

    // The negative flows are done first for the deploy account transaction type. If the positive
    // flow will occure first, the deploy will succeed, and the nonce of the deployed contract will
    // be advanced. This will cause the negative flow to fail, as the nonce of a deployed contract
    // must be zero.
    let state = &mut test_state(block_context, account_balance, &[(faulty_account, 5)]);
    // Negative flows.
    ...
    // let sender_address = faulty_account.get_instance_address(3);
    let account_tx = create_account_tx_for_validate_test(
        tx_type,
        CALL_CONTRACT,
        Some(*FeatureContract::TestContract(CairoVersion::Cairo0).get_instance_address(0).0.key()),
        &mut NonceManager::default(),
        sender_address,
    );
    let error = account_tx.execute(state, block_context, true, true).unwrap_err();
    ...
    if tx_type == TransactionType::DeployAccount {
        ...
        // let sender_address = faulty_account.get_instance_address(4);
    ...
    // Valid logic.
    let sender_address = faulty_account.get_instance_address(1);
    let account_tx = create_account_tx_for_validate_test(
        tx_type,
        VALID,
        None,
    ...
    if tx_type != TransactionType::DeployAccount {
        ...
        let sender_address = faulty_account.get_instance_address(2);
        let account_tx = create_account_tx_for_validate_test(
            tx_type,
            CALL_CONTRACT,
