...
#[case::with_cairo0_account(
    ExpectedResultTestInvokeTx{
        resources: &get_syscall_resources(DeprecatedSyscallSelector::CallContract) + &ExecutionResources {
            n_steps: 62,
            n_memory_holes:  0,
...
fn test_l1_handler(#[values(false, true)] use_kzg_da: bool) {
    ...
    let mut expected_resource_mapping = ResourcesMapping(HashMap::from([
        ...
        (
            abi_constants::N_STEPS_RESOURCE.to_string(),
            get_transaction_resources(TransactionType::L1Handler).n_steps + 246,
        ),
        (
            RANGE_CHECK_BUILTIN_NAME.to_string(),
            get_transaction_resources(TransactionType::L1Handler).builtin_instance_counter
                [&RANGE_CHECK_BUILTIN_NAME.to_string()]
                + 6,
        ),
