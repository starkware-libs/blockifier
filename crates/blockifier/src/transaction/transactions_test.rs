...
use crate::test_utils::{
    ...
    test_erc20_sequencer_balance_key, DictStateReader, NonceManager, BALANCE, CHAIN_ID_NAME,
...
use crate::transaction::constants::{self, QUERY_VERSION_BASE_BIT};
...
#[test]
fn test_simulate_flag() {
    let state = &mut create_state_with_cairo1_account();
    let block_context = &BlockContext::create_for_account_testing();

    let query_version_base = BigUint::from(2_u8).pow(QUERY_VERSION_BASE_BIT);
    let version = query_version_base + BigUint::from(1_u8);
