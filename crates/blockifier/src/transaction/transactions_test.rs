...
fn test_insufficient_resource_bounds(state: &mut CachedState<DictStateReader>) {
    ...
    // Test V3 tx
    let current_strk_l1_gas_price = block_context.gas_prices.strk_l1_gas_price;
    ...
    resource_bounds: l1_resource_bounds(insufficient_max_l1_gas_amount,
        current_strk_l1_gas_price),
    ...
    let insufficient_max_l1_gas_price = current_strk_l1_gas_price - 1;
    let invalid_v3_tx = account_invoke_tx(invoke_tx_args! {
    ...
    assert_matches!(
        execution_error,
        TransactionExecutionError::MaxL1GasPriceTooLow{ max_l1_gas_price, current_l1_gas_price }
        if max_l1_gas_price == Fee(insufficient_max_l1_gas_price) &&
        current_l1_gas_price == Fee(current_strk_l1_gas_price)
    );
}
...
fn test_invalid_nonce(state: &mut CachedState<DictStateReader>) {
    ...
    transactional_state.increment_nonce(invalid_tx_context.sender_address()).unwrap();
    let pre_validation_err = invalid_tx
        .perform_pre_validation_checks(
            &mut transactional_state,
            &invalid_tx_context,
