...
fn expected_fee_transfer_call_info(
    ...
    let sender_balance_key_low =
        get_fee_token_var_address(&account_address).expect("Cannot get sender balance low key.");
...
fn test_deploy_account_tx(
    ...
    let deployed_account_balance_key =
        get_fee_token_var_address(&contract_address!(*deployed_account_address.0.key())).unwrap();
