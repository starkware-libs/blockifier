...
fn test_max_fee_exceeds_balance(state: &mut CachedState<DictStateReader>) {
    ...
    // Invoke.
    let valid_tx = invoke_tx();
    let invalid_tx = AccountTransaction::Invoke(
        InvokeTransactionV1 {
            account_params: AccountParams { max_fee: invalid_max_fee, ..valid_tx.account_params },
            ..valid_tx
        }
        .into(),
    );
    ...
    // Declare.
    let valid_tx = declare_tx(TEST_EMPTY_CONTRACT_CLASS_HASH, TEST_ACCOUNT_CONTRACT_ADDRESS, None);
