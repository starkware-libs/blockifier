...
fn test_state_get_fee_token_balance(state: &mut CachedState<DictStateReader>) {
    ...
    AccountTransaction::Invoke(mint_tx.into()).execute(state, block_context, true).unwrap();
...
fn test_negative_invoke_tx_flows(state: &mut CachedState<DictStateReader>) {
    ...
    // Test revert error.
    assert!(execution_result.revert_error.unwrap().starts_with("Insufficient max fee:"));
