...
fn expected_validate_call_info(
    ...
    Some(CallInfo {
        ...
        execution: CallExecution {
            retdata: (expected_returned_data),
            ..Default::default()
        },
        vm_resources,
        ..Default::default()
...
fn test_declare_tx(
    ...
    let expected_returned_data = if account_path.contains("cairo1") {
        retdata!(stark_felt!( // Returned data is VALIDATED
...
fn test_deploy_account_tx(
    ...
    let expected_actual_resources = if account_path.contains("cairo1") {
        ResourcesMapping(HashMap::from([
            ...
            (RANGE_CHECK_BUILTIN_NAME.to_string(), 87),
            (abi_constants::N_STEPS_RESOURCE.to_string(), 3716),
        ]))
    } else {
        ResourcesMapping(HashMap::from([
            (abi_constants::GAS_USAGE.to_string(), 3060),
