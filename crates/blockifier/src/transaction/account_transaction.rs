...
impl AccountTransaction {
    ...
    fn concurrency_execute_fee_transfer<U: UpdatableState>(
        ...
        let cache = transfer_state.cached_state.cache.get_mut();
        ...
        let storage_writes = &mut transfer_state.cached_state.cache.get_mut().writes.storage;
    ...
    fn run_non_revertible<U: UpdatableState>(
        ...
        let tx_receipt = TransactionReceipt::from_account_tx(
            self,
            &tx_context,
            &state.cached_state.get_actual_state_changes()?,
            &resources,
    ...
    fn run_revertible<U: UpdatableState>(
        ...
        let validate_state_changes = state.cached_state.get_actual_state_changes()?;
            ...
            Ok(execute_call_info) => {
                ...
                let tx_receipt = TransactionReceipt::from_account_tx(
                    ...
                    &StateChanges::merge(vec![
                        validate_state_changes,
                        execution_state.cached_state.get_actual_state_changes()?,
                    ]),
                    &execution_resources,
                    validate_call_info.iter().chain(execute_call_info.iter()),
                    0,
