...
use crate::execution::entry_point::{
    CallEntryPoint, CallInfo, ExecutionResourcesManager, StateContext,
};
...
impl AccountTransaction {
    ...
    fn validate_tx(
        ...
        let validate_call = CallEntryPoint {
            ...
            class_hash: None,
            storage_address: ctx.account_tx_context.sender_address,
            caller_address: ContractAddress::default(),
        };
        let validate_call_info = validate_call.execute(ctx)?;
    ...
    fn charge_fee(
        ...
        if ctx.account_tx_context.max_fee == no_fee {
            // Fee charging is not enforced in some tests.
            return Ok((no_fee, None));
        }

        let actual_fee = calculate_tx_fee(ctx.block_context);
        let fee_transfer_call_info = Self::execute_fee_transfer(ctx, actual_fee)?;
    ...
    fn execute_fee_transfer(
        ...
        let fee_transfer_call = CallEntryPoint {
            ...
            storage_address: ctx.block_context.fee_token_address,
            caller_address: ctx.account_tx_context.sender_address,
        };

        Ok(fee_transfer_call.execute(ctx)?)
    }
}
    ...
    fn execute_raw(
        ...
        match self {
            Self::Declare(ref tx, ref mut contract_class) => {
                ...
                // Validate.
                validate_call_info = self.validate_tx(ctx)?;

                // Execute.
                execute_call_info = tx.run_execute(ctx, contract_class)?;
            }
            Self::DeployAccount(ref tx) => {
                // Execute the constructor of the deployed class.
                execute_call_info = tx.run_execute(ctx, None)?;
        ...
        let (actual_fee, fee_transfer_call_info) = Self::charge_fee(ctx)?;
