...
impl AccountTransaction {
    ...
    fn enforce_fee(&self,account_tx_context: &AccountTransactionContext, block_context: &BlockContext) -> bool {
        account_tx_context.max_fee(block_context.gas_prices) != Fee(0)
    }
    ...
    fn check_fee_balance<S: StateReader>(
        ...
        // Check fee balance.
        if self.enforce_fee(&account_tx_context, block_context) {
    ...
    fn calculate_actual_fee_and_resources(
        ...
        *actual_resources.0.get_mut(&abi_constants::N_STEPS_RESOURCE.to_string()).unwrap() +=
        ...
        if is_reverted || account_tx_context.max_fee(block_context.gas_prices) == Fee(0) {
            // We cannot charge more than max_fee for reverted txs.
            actual_fee = min(actual_fee, account_tx_context.max_fee(block_context.gas_prices));
        }
