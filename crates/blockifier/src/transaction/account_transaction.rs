...
impl AccountTransaction {
    ...
    fn charge_fee(
        ...
        let actual_fee = calculate_tx_fee(context.block_context);
        let fee_transfer_call_info = Self::execute_fee_transfer(context, actual_fee)?;
    ...
    fn execute_fee_transfer(
        ...
        let fee_transfer_call = CallEntryPoint {
            ...
            storage_address: context.block_context.fee_token_address,
...
impl<S: StateReader> ExecutableTransaction<S> for AccountTransaction {
    fn execute_raw(
        ...
        let mut context = ExecutionContext::new(state, block_context, &account_tx_context);
        match self {
            Self::Declare(ref tx, ref mut contract_class) => {
                let contract_class = Some(mem::take(contract_class));

                // Validate.
                validate_call_info = self.validate_tx(&mut context)?;

                // Execute.
                execute_call_info = tx.run_execute(&mut context, contract_class)?;
            }
            Self::DeployAccount(ref tx) => {
                // Execute the constructor of the deployed class.
                execute_call_info = tx.run_execute(&mut context, None)?;
        ...
        let (actual_fee, fee_transfer_call_info) = Self::charge_fee(&mut context)?;
