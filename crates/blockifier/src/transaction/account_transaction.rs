...
impl AccountTransaction {
    ...
    fn create_call_info(
        ...
        let sender_balance_key = get_fee_token_var_address(tx_info.sender_address());
        let sender_balance_value = state
            .get_storage_at(
                block_context.chain_info.fee_token_address(&tx_info.fee_type()),
                sender_balance_key,
            )
            .unwrap();
        let balance: u128 = felt_to_u128(&stark_felt_to_felt(sender_balance_value)).unwrap();
    ...
    pub fn transfer(
        ...
        assert_ne!(sender, ContractAddress::from(0_u8), "sender cannot be zero");
        assert_ne!(recipient, ContractAddress::from(0_u8), "recipient cannot be zero");
        AccountTransaction::add_amount_to_balance(
            state,
            fee_token_address,
            sender,
