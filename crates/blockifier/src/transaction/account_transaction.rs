...
use crate::fee::fee_utils::{
    calculate_tx_l1_gas_usage, can_pay_fee, fee_for_l1_gas_limit, verify_can_pay_max_fee,
};
...
impl AccountTransaction {
    ...
    // TODO(Dori, 1/12/2023): If `charge_fee` is false, no point in calling this function.
    fn post_execution_final_fee_and_error<S: StateReader>(
        ...
        // pre-validation checks: deprecated transactions check `balance >= max_fee`, and current
        // transactions check `balance >= max_l1_gas * gas_price`.
        match account_tx_context {
            ...
            AccountTransactionContext::Current(current_context) => {
                ...
                if l1_gas_usage > max_l1_gas {
                    return Ok((
                        fee_for_l1_gas_limit(current_context, block_context),
                        Some(format!(
        ...
        // The resource bounds are verified to be covered by the account's balance in pre-validation
        // checks, and account validation can only effect the account contract's state (so balance
        // post-validation is equal to balance pre-validation). As such, it is safe to charge
        // post-execution fee if (a) resources used (and the fee derived from them) are within
        // bounds and (b) the execution state is reverted.
        Ok((
            post_execute_fee,
