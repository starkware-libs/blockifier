        ...
        match self {
            ...
            Self::DeployAccount(tx) => {
                let validate_calldata = concat(vec![
                    vec![tx.tx.class_hash.0, tx.tx.contract_address_salt.0],
                    (*tx.tx.constructor_calldata.0).clone(),
                ]);
    ...
    fn get_account_transaction_context(&self) -> AccountTransactionContext {
        match self {
            ...
            Self::DeployAccount(tx) => AccountTransactionContext {
                transaction_hash: tx.tx.transaction_hash,
                max_fee: tx.tx.max_fee,
                version: tx.tx.version,
                signature: tx.tx.signature.clone(),
                nonce: tx.tx.nonce,
