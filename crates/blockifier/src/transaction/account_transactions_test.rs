...
fn test_count_actual_storage_changes(
    ...
    let execution_info =
        account_tx.execute_raw(&mut state, &block_context, excution_flags.clone()).unwrap();
    ...
    let execution_info =
        account_tx.execute_raw(&mut state, &block_context, excution_flags).unwrap();
...
fn test_concurrency_execute_fee_transfer(
    ...
    let result = account_tx
        .execute_raw(&mut transactional_state, &block_context, excution_flags.clone())
        .unwrap();
    assert!(!result.is_reverted());
    ...
    let execution_result =
        account_tx.execute_raw(&mut transactional_state, &block_context, excution_flags);
