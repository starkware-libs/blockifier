...
fn test_max_fee_limit_validate(
    ...
    let class_info = calculate_class_info_for_testing(grindy_validate_account.get_class()).unwrap();
...
fn test_fail_declare(block_context: BlockContext, max_fee: Fee) {
    ...
    let class_info = calculate_class_info_for_testing(contract_class).unwrap();
