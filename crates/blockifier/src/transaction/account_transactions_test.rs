...
use starknet_api::transaction::{
    ...
    ResourceBoundsMapping, TransactionHash, TransactionSignature, TransactionVersion,
};
...
/// Test the interaction between the different flavors of execution.
#[rstest]
#[case(TransactionVersion::ONE, FeeType::Eth, true)]
#[case(TransactionVersion::THREE, FeeType::Strk, false)]
fn test_simulate_validate_charge_interaction(
    ...
    let gas_and_fee = |base_gas: u64| {
        // Validation incurs a constant gas overhead.
        let gas = base_gas + if validate { 21 } else { 0 };
        (gas, get_fee_by_l1_gas_usage(&block_context, gas as u128, &fee_type))
    };
    ...
    let pre_validation_base_args = invoke_tx_args! {
        max_fee,
        resource_bounds: l1_resource_bounds(MAX_L1_GAS_AMOUNT as u64, MAX_L1_GAS_PRICE),
        sender_address: account_address,
        calldata: calldata![
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(6582);
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(31336);
    let result = account_invoke_tx(invoke_tx_args! {
        max_fee,
        resource_bounds: l1_resource_bounds(MAX_L1_GAS_AMOUNT as u64, MAX_L1_GAS_PRICE),
    ...
    let (revert_gas_used, revert_fee) = gas_and_fee(5873);
    ...
    let (gas_bound, fee_bound) = gas_and_fee(5830);
    let (limited_gas_used, limited_fee) = gas_and_fee(8278);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10574);
    let tx_execution_info = account_invoke_tx(invoke_tx_args! {
        max_fee: fee_bound,
    ...
    let (huge_gas_limit, huge_fee) = gas_and_fee(100000);
    ...
    let (just_not_enough_gas_bound, just_not_enough_fee_bound) = gas_and_fee(6000);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10574);
