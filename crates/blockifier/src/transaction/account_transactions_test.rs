...
#[rstest]
#[case(TransactionVersion::ZERO, "DUMMY", false)]
...
fn test_revert_on_resource_overuse(
    ...
    let mut write_a_lot_calldata = || {
        ...
        create_calldata(
            contract_address,
            "write_a_lot",
            &[stark_felt!(n_writes), stark_felt!(write_a_lot_start_address)],
        )
    };
    ...
    if is_revertible {
        ...
        assert_matches!(
            ...
            TransactionExecutionError::PostExecutionAuditorError(
                PostExecutionAuditorError::MaxFeeExceeded { max_fee, actual_fee: fee_in_error }
            )
