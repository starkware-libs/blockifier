...
fn test_simulate_validate_charge_interaction(
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(6696);
    ...
    let (actual_gas_used, actual_fee) = gas_and_fee(31450);
    let result = account_invoke_tx(invoke_tx_args! {
        max_fee,
        resource_bounds: l1_resource_bounds(MAX_L1_GAS_AMOUNT, MAX_L1_GAS_PRICE),
    ...
    let (revert_gas_used, revert_fee) = gas_and_fee(5987);
    ...
    let (gas_bound, fee_bound) = gas_and_fee(5944);
    let (limited_gas_used, limited_fee) = gas_and_fee(8392);
    let (unlimited_gas_used, unlimited_fee) = gas_and_fee(10688);
    let tx_execution_info = account_invoke_tx(invoke_tx_args! {
        max_fee: fee_bound,
