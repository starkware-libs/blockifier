...
fn test_enforce_fee_false_works(
    ...
    let TestInitData {
        ...
        mut nonce_manager,
        block_context,
    } = create_test_init_data(max_fee, block_context.clone(), create_state(block_context));
...
fn test_max_fee_limit_validate(
    ...
    let TestInitData {
        ...
        mut nonce_manager,
        block_context,
    } = create_test_init_data(Fee(MAX_FEE), block_context.clone(), create_state(block_context));
...
fn test_reverted_reach_steps_limit(
    ...
    let state = create_state(block_context.clone());
    // Limit the number of execution steps (so we quickly hit the limit).
