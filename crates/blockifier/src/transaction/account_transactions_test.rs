...
use crate::transaction::test_utils::{
    ...
    TestInitData, INVALID, VALIDATE_WITHOUT_WRITE, VALIDATE_WITH_WRITE,
};
...
fn test_count_actual_storage_changes_with_storage_writes_in_validation(
    ...
    let chain_info = &block_context.chain_info;
    let account_contract = FeatureContract::TestAccount(cairo_version);
    ...
    let scenario = VALIDATE_WITHOUT_WRITE;
    ...
    let scenario = VALIDATE_WITH_WRITE;
    ...
    let execution_info = account_tx.execute(&mut state, &block_context, true, true);
    let da_gas_usage_reset_storage_writes = execution_info.unwrap().da_gas;

    assert_eq!(da_gas_usage_no_storage_writes, da_gas_usage_reset_storage_writes);
