...
fn create_test_init_data(
    ...
    account_tx.execute(&mut state, &block_context, false).unwrap();
...
fn test_fee_enforcement(
    ...
    for max_fee_value in 0..2 {
        ...
        let result = account_tx.execute(&mut state, &block_context, false);
...
fn test_infinite_recursion(
    ...
    let tx_execution_info = account_tx.execute(&mut state, &block_context, false).unwrap();
    if success {
...
fn test_revert_invoke(
    ...
    let account_tx = AccountTransaction::DeployAccount(deploy_account_tx);
    let deploy_execution_info = account_tx.execute(&mut state, &block_context, false).unwrap();
...
fn test_fail_deploy_account(block_context: BlockContext) {
    ...
    deploy_account_tx.execute(&mut state, &block_context, false).unwrap_err();
...
fn test_fail_declare(max_fee: Fee, #[from(create_test_init_data)] init_data: TestInitData) {
    ...
    declare_account_tx.execute(&mut state, &block_context, false).unwrap_err();
...
fn test_max_fee_to_max_steps_conversion(
    ...
    let tx_execution_info1 = account_tx1.execute(&mut state, &block_context, false).unwrap();
    ...
    let tx_execution_info2 = account_tx2.execute(&mut state, &block_context, false).unwrap();
