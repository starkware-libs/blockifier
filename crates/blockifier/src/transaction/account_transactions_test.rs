...
use crate::transaction::constants::{EXECUTE_ENTRY_POINT_NAME, TRANSFER_ENTRY_POINT_NAME};
...
fn test_contract_ctor_frame_stack_trace(
    ...
    let deploy_contract_selector = selector_from_name("deploy_contract");
...
0: Error in the called contract (contract address: {account_address_felt}, class hash: \
    ...
    println!("DORI: {cairo_version:?} execute: {execute_offset}",);
    println!("DORI: {cairo_version:?} deploy: {deploy_offset}",);
    println!("DORI: {cairo_version:?} ctor: {ctor_offset}",);
