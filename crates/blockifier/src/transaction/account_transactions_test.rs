...
fn create_test_init_data(
    ...
    let deployed_account_balance_key =
        get_erc20_balance_var_address(&contract_address!(*account_address.0.key())).unwrap();
...
fn test_revert_invoke(
    ...
    // can pay for the transaction execution.
    let deployed_account_balance_key =
        get_erc20_balance_var_address(&contract_address!(*deployed_account_address.0.key()))
            .unwrap();
    state.set_storage_at(fee_token_address, deployed_account_balance_key, stark_felt!(BALANCE));
    ...
    assert_eq!(
        state
            .get_fee_token_balance(&block_context, &deployed_account_address)
            .unwrap()
            .to_biguint(),
...
fn test_revert_on_overdraft(
    ...
    let balance =
        state.get_fee_token_balance(&block_context, &account_address).unwrap().to_biguint();
    ...
    let execution_info = write_and_transfer(
        ...
        felt_to_stark_felt(&Felt252::from(&balance)), // transfer the entire balance.
        account_address,
        contract_address,
        &block_context,
        &mut nonce_manager,
    ...
    let expected_new_balance = balance - transfer_tx_fee.0;
    ...
    assert_eq!(
        state.get_fee_token_balance(&block_context, &recipient_address).unwrap().to_biguint(),
