...
fn test_revert_invoke(
    ...
    let account_tx = AccountTransaction::DeployAccount(deploy_account_tx);
    let account_tx_context = account_tx.get_account_tx_context(&block_context);
...
fn test_fail_declare(max_fee: Fee, #[from(create_test_init_data)] init_data: TestInitData) {
    ...
    let account_tx_context = declare_account_tx.get_account_tx_context(&block_context);
...
fn test_max_fee_to_max_steps_conversion(
    ...
    let execution_context1 = EntryPointExecutionContext::new_invoke(
        &block_context,
        &account_tx1.get_account_tx_context(&block_context),
    );
    ...
    let execution_context2 = EntryPointExecutionContext::new_invoke(
        &block_context,
        &account_tx2.get_account_tx_context(&block_context),
    );
...
fn test_revert_on_overdraft(
    ...
    let account_tx_context = approve_tx.get_account_tx_context(&block_context);
