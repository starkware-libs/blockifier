...
fn test_count_actual_storage_changes_with_storage_writes_in_validation(
    ...
    let da_gas_usage_reset_storage_writes = execution_info.da_gas;

    assert_eq!(da_gas_usage_no_storage_writes, da_gas_usage_reset_storage_writes);

    // TODO(Aner, 22/2/2024) Scenario: Only execution changes same storage to a new
    ...
    // TODO(Aner, 22/2/2024) Scenario: Only validation changes same storage to a new
    // value and then back to original value, execution passes.
    // Expected: No storage changes, da_gas as if no writes happened.
