...
fn test_concurrency_execute_fee_transfer(
    ...
    let result = account_tx.execute_raw(&mut transactional_state, &block_context, charge_fee, validate, run_concurrently).unwrap();
    assert!(!result.is_reverted());
    ...
    let mut outer_transactional_state = TransactionalState::create_transactional(&mut transactional_state);
    let execution_result =
        account_tx.execute_raw(&mut outer_transactional_state, &block_context, charge_fee, validate, run_concurrently);
