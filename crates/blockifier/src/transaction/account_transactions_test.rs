...
fn test_insufficient_max_fee_to_insufficient_steps(
    ...
    let execute_calldata = calldata![
        *contract_address.0.key(),       // Contract address.
        ...
        stark_felt!(1_u16),              // Calldata length.
        stark_felt!(1_u16)               // Calldata: depth.
    ];
    let tx1 = invoke_tx(execute_calldata, account_address, Fee(MAX_FEE), None);
    let account_tx1: AccountTransaction =
        AccountTransaction::Invoke(InvokeTransaction::V1(InvokeTransactionV1 {
            nonce: nonce_manager.next(account_address),
            ..tx1
    ...
    let execute_calldata = calldata![
        *contract_address.0.key(),       // Contract address.
        selector_from_name("recurse").0, // EP selector.
        stark_felt!(1_u16),              // Calldata length.
        stark_felt!(300_u16)             // Calldata: depth.
    ];

    let tx2 = invoke_tx(execute_calldata, account_address, actual_fee_depth1, None);
    let account_tx2: AccountTransaction =
        AccountTransaction::Invoke(InvokeTransaction::V1(InvokeTransactionV1 {
            nonce: nonce_manager.next(account_address),
            ..tx2
