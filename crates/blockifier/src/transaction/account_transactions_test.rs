...
impl NonceManager {
    pub fn new(felt: StarkFelt) -> Self {
        Self { next_nonce: stark_felt_to_felt(felt) }
    }

    pub fn next(&mut self) -> Nonce {
        let next = Nonce(felt_to_stark_felt(&self.next_nonce));
        self.next_nonce += Felt252::one();
        next
    }
}
...
fn create_test_state(max_fee: Fee, block_context: &BlockContext) -> TestInitData {
    ...
    TestInitData {
        ...
        account_address,
        contract_address,
        nonce_manager: NonceManager::new(stark_felt!(3_u8)),
    }
}
...
fn test_infinite_recursion() {
    ...
    // Two types of recursion: one "normal" recursion, and one that uses the call_contract syscall.
    let contract_address_felt = *contract_address.0.key();
    let normal_entry_point_selector_felt = selector_from_name("recurse").0;
    let syscall_entry_point_selector_felt = selector_from_name("recursive_syscall").0;

    let normal_calldata = |recursion_depth: u32| -> Calldata {
        calldata![
            contract_address_felt,
            normal_entry_point_selector_felt,
            stark_felt!(1_u8),
    ...
    let syscall_calldata = |recursion_depth: u32| -> Calldata {
        calldata![
            contract_address_felt,
            syscall_entry_point_selector_felt,
    ...
    [(1_u32, true, true), (1000_u32, false, true), (3_u32, true, false), (1000_u32, false, false)]
        ...
        .map(|(recursion_depth, should_be_ok, use_normal_calldata)| {
            ...
            if should_be_ok {
                ...
                assert!(tx_execution_info.revert_error.unwrap().contains(
