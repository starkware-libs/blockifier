...
#[test]
/// Failing account deployment should not change state (no fee charge or nonce bump)
fn test_fail_deploy_account() {
    let mut state = create_state_with_falliable_validation_account();
    let block_context = &BlockContext::create_for_account_testing();

    let deployed_account_address = ContractAddress(
        PatriciaKey::try_from(StarkFelt::try_from(TEST_FAULTY_ACCOUNT_CONTRACT_ADDRESS).unwrap())
            .unwrap(),
    );
    let initial_balance =
        state.get_fee_token_balance(block_context, &deployed_account_address).unwrap();
