...
impl GasVector {
    ...
    pub fn saturated_cost(&self, gas_price: u128, blob_gas_price: u128) -> Fee {
        let l1_gas_cost = match self.l1_gas.checked_mul(gas_price) {
            Some(cost) => cost,
            None => {
                log::warn!(
        ...
        let l1_data_gas_cost = match self.blob_gas.checked_mul(blob_gas_price) {
            Some(cost) => cost,
            None => {
                log::warn!(
        ...
        let total = match l1_gas_cost.checked_add(l1_data_gas_cost) {
            Some(cost) => cost,
            None => {
                log::warn!(
