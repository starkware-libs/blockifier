...
fn get_json_value_without_dafaults() -> serde_json::Value {
    ...
    let mut json_value_without_dafualts: Value = serde_json::from_str(json_data).unwrap();
    json_value_without_dafualts
        .as_object_mut()
        .unwrap()
...
fn test_default_values() {
    let json_value_without_dafualts = get_json_value_without_dafaults();

    let versioned_constants: VersionedConstants =
        serde_json::from_value(json_value_without_dafualts).unwrap();
