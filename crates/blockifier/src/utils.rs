...
/// Returns the result of subtracting the key-value set of `rhs` from the key-value set of `lhs`.
/// (a key that appears in `rhs` with a different value, will not be removed from `lhs`).
/// If `lhs` keys are not a subset of `rhs` keys the function returns an error.
/// Usage: Get updated items from a mapping.
