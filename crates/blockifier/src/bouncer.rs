...
impl From<HashMapWrapper> for BuiltinCount {
    fn from(mut data: HashMapWrapper) -> Self {
        // TODO(yael 24/3/24): replace the unwrap_or_default with expect, once the ExecutionResources
...
impl Bouncer {
    ...
    pub fn try_update<S: StateReader>(
        ...
        if max_capacity.checked_sub(self.accumulated_capacity + tx_weights).is_none() {
