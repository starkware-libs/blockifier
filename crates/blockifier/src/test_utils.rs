...
pub fn get_test_contract_class() -> ContractClass {
    ContractClassV0::from_path(TEST_CONTRACT_PATH).into()
}
...
fn get_class_hash_to_v0_class_mapping() -> ContractClassMapping {
    HashMap::from([
        ...
        (
            ...
            ContractClassV0::from_path(SECURITY_TEST_CONTRACT_PATH).into(),
        ),
        (
            ClassHash(stark_felt!(TEST_EMPTY_CONTRACT_CLASS_HASH)),
            ContractClassV0::from_path(TEST_EMPTY_CONTRACT_PATH).into(),
        ),
    ])
}
...
fn get_class_hash_to_v1_class_mapping() -> ContractClassMapping {
    HashMap::from([
        (
            ClassHash(stark_felt!(TEST_CLASS_HASH)),
            ContractClassV1::from_path(TEST_CONTRACT_CAIRO1_PATH).into(),
        ),
        (
            ClassHash(stark_felt!(TEST_EMPTY_CONTRACT_CLASS_HASH)),
            ContractClassV1::from_path(TEST_EMPTY_CONTRACT_CAIRO1_PATH).into(),
        ),
    ])
}
...
impl ContractClassV0 {
    pub fn from_path(contract_path: &str) -> ContractClassV0 {
...
impl ContractClassV1 {
    pub fn from_path(contract_path: &str) -> ContractClassV1 {
        let raw_contract_class = get_raw_contract_class(contract_path);
        Self::try_from_json_string(&raw_contract_class).unwrap()
    }
}
