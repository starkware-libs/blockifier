...
#[macro_export]
macro_rules! inner_exc_for_custom_hint {
...
#[macro_export]
macro_rules! inner_exc_for_invalid_scenario {
    ($inner_exc:expr) => {
...
macro_rules! check_entry_point_execution_error {
    ($error:expr, $expected_hint:expr $(,)?) => {
        ...
        {
            match $expected_hint {
                Some(expected_hint) => $crate::inner_exc_for_custom_hint!(inner_exc, expected_hint),
                None => $crate::inner_exc_for_invalid_scenario!(inner_exc),
            };
        } else {
