...
impl<S: StateReader> VersionedStateProxy<S> {
    ...
    fn lock_state(&self) -> MutexGuard<'_, VersionedState<S>> {
        self.state.lock().expect(LOCK_ERR_MESSAGE)
    }
}
...
impl<S: StateReader> StateReader for VersionedStateProxy<S> {
    fn get_storage_at(
        ...
        self.lock_state().get_storage_at(self.version, contract_address, key)
    }

    fn get_nonce_at(&self, contract_address: ContractAddress) -> StateResult<Nonce> {
        self.lock_state().get_nonce_at(self.version, contract_address)
    }

    fn get_class_hash_at(&self, contract_address: ContractAddress) -> StateResult<ClassHash> {
        self.lock_state().get_class_hash_at(self.version, contract_address)
    }

    fn get_compiled_class_hash(&self, class_hash: ClassHash) -> StateResult<CompiledClassHash> {
        self.lock_state().get_compiled_class_hash(self.version, class_hash)
    }

    fn get_compiled_contract_class(&self, class_hash: ClassHash) -> StateResult<ContractClass> {
        let contract_class =
            self.lock_state().get_compiled_contract_class(self.version, class_hash);
