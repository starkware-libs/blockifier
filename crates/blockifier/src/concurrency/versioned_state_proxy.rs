...
impl<S: StateReader> VersionedState<S> {
    ...
    pub fn commit_chunk<T>(&mut self, upper_index: TxIndex, block_state: &mut CachedState<T>)
    where
        T: StateReader,
    {
        // Storage
        for (&(contract_address, storage_key), _) in self.storage.get_writes().iter() {
            let value = self.storage.read(upper_index, (contract_address, storage_key)).unwrap();
        ...
        for (&contract_address, map) in self.nonces.get_writes().iter() {
            ...
            let current_nonce = self.nonces.read(upper_index, contract_address).unwrap();
        ...
        for (&contract_address, _) in self.class_hashes.get_writes().iter() {
            let value = self.class_hashes.read(upper_index, contract_address).unwrap();
        ...
        for (&class_hash, _) in self.compiled_class_hashes.get_writes().iter() {
            let value = self.compiled_class_hashes.read(upper_index, class_hash).unwrap();
        ...
        for (&class_hash, _) in self.compiled_contract_classes.get_writes().iter() {
            let value = self.compiled_contract_classes.read(upper_index, class_hash).unwrap();
