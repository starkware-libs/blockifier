...
impl<S: StateReader> VersionedState<S> {
    ...
    pub fn get_newest_writes_as_of_index(&mut self, tx_index: TxIndex) -> StateMaps {
        StateMaps {
            storage: self.storage.get_newest_writes_as_of_index(tx_index),
            nonces: self.nonces.get_newest_writes_as_of_index(tx_index),
            class_hashes: self.class_hashes.get_newest_writes_as_of_index(tx_index),
            compiled_class_hashes: self
                .compiled_class_hashes
                .get_newest_writes_as_of_index(tx_index),
    ...
    where
        T: StateReader,
    {
        let writes = self.get_newest_writes_as_of_index(from_index);
        parent_state.update_cache(writes);

        parent_state.update_contract_class_cache(
            self.compiled_contract_classes.get_newest_writes_as_of_index(from_index),
        );
    }
    ...
    fn delete_writes(
        ...
        for &key in writes.storage.keys() {
            self.storage.delete_writes(key, tx_index);
        }
        for &key in writes.nonces.keys() {
            self.nonces.delete_writes(key, tx_index);
        }
        for &key in writes.class_hashes.keys() {
            self.class_hashes.delete_writes(key, tx_index);
        }
        for &key in writes.compiled_class_hashes.keys() {
            self.compiled_class_hashes.delete_writes(key, tx_index);
        }
        for &key in class_hash_to_class.keys() {
            self.compiled_contract_classes.delete_writes(key, tx_index);
        }
    }
}
