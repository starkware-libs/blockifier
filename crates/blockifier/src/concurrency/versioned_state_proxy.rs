...
impl<S: StateReader> VersionedState<S> {
    ...
    pub fn validate_read_set(&mut self, tx_index: usize, state_cache: &mut StateCache) -> bool {
        // If the tx_index is 0, then the read set is valid. Since tx_index 0 has no predecessors,
...
impl<S: StateReader> ThreadSafeVersionedState<S> {
    pub fn pin_version(&self, tx_index: usize) -> VersionedStateProxy<S> {
