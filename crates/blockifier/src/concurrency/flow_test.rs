...
fn scheduler_flow_test(
    ...
    // This test simulates DEFAULT_CHUNK_SIZE txs. Each advances a counter in the storage entry
    // (contract_address, storage_key). Notice that every tx is dependent in its predecessor since
            ...
            loop {
                ...
                task = match task {
                    Task::ExecutionTask(tx_index) => {
                        ...
                        // Simulate a read operation of a tx.
                        let counter = SierraU128::from_storage(
                            &mut state_proxy,
                            &contract_address,
                            &storage_key,
                        )
