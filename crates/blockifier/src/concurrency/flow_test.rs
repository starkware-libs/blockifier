    ...
    match task {
        Task::ExecutionTask(tx_index) => {
            let state_proxy = match tx_index {
                0 => {
                    return (
                        state_maps_with_single_storage_entry(contract_address, storage_key, 0),
                        state_maps_with_single_storage_entry(contract_address, storage_key, 1),
                    );
                }
                _ => versioned_state.pin_version(tx_index - 1),
            };
            let tx_written_value =
                SierraU128::from_storage(&state_proxy, &contract_address, &storage_key)
                    .unwrap()
                    .as_value();
            (
                state_maps_with_single_storage_entry(
                    contract_address,
                    storage_key,
                    tx_written_value,
                ),
