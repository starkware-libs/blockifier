...
use crate::concurrency::validator::{StorageEntry, StorageType, Validator};
...
fn test_versioned_state() {
    ...
    let mut versioned_state = VersionedState::new(cached_state_static);
    ...
    let res = versioned_state.set_storage_at(contract_address, key, stark_felt!(23_u8), 0);
    assert!(res.is_ok());

    let res = versioned_state.set_nonce_at(contract_address, nonce, 0);
    assert!(res.is_ok());

    let res = versioned_state.set_class_hash_at(contract_address, class_hash, 0);
    assert!(res.is_ok());

    let res = versioned_state.set_compiled_class_hash_at(class_hash, compiled_class_hash, 0);
    assert!(res.is_ok());

    // Write new data.
    let compiled_class_hash_v5 = CompiledClassHash(stark_felt!(100_u8));
    let res = versioned_state.set_compiled_class_hash_at(class_hash, compiled_class_hash_v5, 5);
    assert!(res.is_ok());

    let compiled_class_hash_v3 = CompiledClassHash(stark_felt!(5_u8));
    let res = versioned_state.set_compiled_class_hash_at(class_hash, compiled_class_hash_v3, 3);
    assert!(res.is_ok());
    ...
    assert!(!validator.validate_read_set(0, read_set, &mut versioned_state));
    ...
    assert!(validator.validate_read_set(4, read_set, &mut versioned_state));
    ...
    let read_set: &[StorageEntry] = &[entry_1, entry_2];
    assert!(validator.validate_read_set(12, read_set, &mut versioned_state));
}
