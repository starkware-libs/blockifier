...
fn _add_fee_to_sequencer_balance(
    ...
    actual_fee: &Fee,
    sequencer_key_high: StorageKey,
    sequencer_key_low: StorageKey,
    sequencer_value_high: StarkFelt,
    sequencer_value_low: StarkFelt,
) {
    let felt_fee = Felt252::from(actual_fee.0);
    let new_value_low = stark_felt_to_felt(sequencer_value_low) + felt_fee.clone();
    let overflow = stark_felt_to_felt(sequencer_value_low) > Felt252::max_value() - felt_fee;
    let new_value_high = if overflow {
        stark_felt_to_felt(sequencer_value_high) + Felt252::from(1_u8)
    } else {
    ...
    let writes = StateMaps {
        storage: HashMap::from([
            ((fee_token_address, sequencer_key_low), felt_to_stark_felt(&new_value_low)),
            ((fee_token_address, sequencer_key_high), felt_to_stark_felt(&new_value_high)),
        ]),
        ..StateMaps::default()
    };
