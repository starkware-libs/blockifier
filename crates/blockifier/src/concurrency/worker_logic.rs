...
use crate::state::cached_state::{CachedState, ContractClassMapping, StateMaps, TransactionalState};
...
impl<S: StateReader> WorkerExecutor<S> {
    ...
    fn execute_tx(&self, tx_index: TxIndex) {
        ...
        let mut transactional_state =
            TransactionalState::create_transactional(&mut tx_state);
        let validate = true;
        let charge_fee = true;
        ...
        if execution_result.is_ok() {
            ...
            self.state
                ...
                .apply_writes(&transactional_state.cached_state.cache.borrow().writes, &class_hash_to_class);
        }
