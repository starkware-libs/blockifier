...
impl<S: StateReader> WorkerExecutor<S> {
    ...
    /// If execution failed, this transaction is considered commited.
    /// Else, we fix the call info and update the sequencer balance.
    /// Then, we ask the bouncer if there is room.
    /// Only if there is room we commit the transaction.
