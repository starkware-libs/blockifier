...
impl<'a, S: StateReader> WorkerExecutor<'a, S> {
    ...
    fn execute_tx(&self, tx_index: TxIndex) {
        ...
        if execution_result.is_ok() {
            let class_hash_to_class = transactional_state.state.class_hash_to_class.borrow();
            ...
            self.state.pin_version(tx_index).apply_writes(
                &transactional_state.state.cache.borrow().writes,
                &class_hash_to_class,
            );
        }

        // Write the transaction execution outputs.
        let tx_reads_writes = transactional_state.state.cache.take();
        // In case of a failed transaction, we don't record its writes and visited pcs.
        let (writes, visited_pcs) = match execution_result {
            Ok(_) => (tx_reads_writes.writes, transactional_state.state.visited_pcs),
