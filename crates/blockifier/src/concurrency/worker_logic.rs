...
impl<'a, S: StateReader> WorkerExecutor<'a, S> {
    ...
    fn execute_tx(&self, tx_index: TxIndex) {
        ...
        let execution_result =
            tx.execute_raw(&mut transactional_state, self.block_context, charge_fee, validate, run_concurrently);
    ...
    fn commit_tx(&self, tx_index: TxIndex) -> bool {
        ...
        if let Ok(tx_execution_info) = tx_result.as_mut() {
            ...
            let run_concurrently = true;
            tx_state_changes_keys.update_sequencer_key_in_storage(&tx_context, tx_execution_info, run_concurrently);
            ...
            complete_fee_transfer_flow(&tx_context, tx_execution_info, &mut tx_versioned_state, run_concurrently);
