...
impl<U: UpdatableState> ThreadSafeVersionedState<U> {
    pub fn commit_chunk_and_recover_block_state(self, commit_index: TxIndex) -> U {
        let mut versioned_state = self.into_inner_state();
        if commit_index > 0 {
            let writes = versioned_state.get_writes_up_to_index(commit_index);
            let class_hash_to_class =
                versioned_state.compiled_contract_classes.get_writes_up_to_index(commit_index);
            // TODO(barak, 01/08/2024): Add visited_pcs argument to `apply_writes`.
            versioned_state.initial_state.apply_writes(
                &writes,
                &class_hash_to_class,
                &HashMap::default(),
            );
