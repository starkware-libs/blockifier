...
impl<U: UpdatableState> VersionedState<U> {
    ...
    pub fn into_initial_state(self) -> U {
        self.initial_state
    }
...
impl<U: UpdatableState> ThreadSafeVersionedState<U> {
    ...
    pub fn into_inner_state(self) -> VersionedState<U> {
        Arc::try_unwrap(self.0)
            .unwrap_or_else(|_| {
                panic!(
                    "To consume the versioned state you must to have only one strong reference to \
