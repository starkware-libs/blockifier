...
impl<S: StateReader> VersionedState<S> {
    ...
    fn get_initial_state(&self) -> MutexGuard<'_, S> {
        self.initial_state.lock().expect("Failed to acquire state lock.")
    }
    ...
    fn commit(&mut self, index: TxIndex, parent_state: Arc<Mutex<CachedState<impl StateReader>>>) {
...
impl<S: StateReader> ThreadSafeVersionedState<S> {
    ...
    pub fn commit(
        ...
        self.0.lock().expect("Failed to acquire state lock.").commit(chunk_size, parent_state);
    }
}
...
impl<S: StateReader> StateReader for VersionedStateProxy<S> {
    fn get_storage_at(
        ...
        match state.storage.read(self.tx_index, (contract_address, key)) {
            ...
            None => {
                let initial_value =
                    state.get_initial_state().get_storage_at(contract_address, key)?;
    ...
    fn get_nonce_at(&self, contract_address: ContractAddress) -> StateResult<Nonce> {
        ...
        match state.nonces.read(self.tx_index, contract_address) {
            Some(value) => Ok(value),
            None => {
                let initial_value = state.get_initial_state().get_nonce_at(contract_address)?;
    ...
    fn get_class_hash_at(&self, contract_address: ContractAddress) -> StateResult<ClassHash> {
        ...
        match state.class_hashes.read(self.tx_index, contract_address) {
            ...
            None => {
                let initial_value =
                    state.get_initial_state().get_class_hash_at(contract_address)?;
    ...
    fn get_compiled_class_hash(&self, class_hash: ClassHash) -> StateResult<CompiledClassHash> {
        ...
        match state.compiled_class_hashes.read(self.tx_index, class_hash) {
            ...
            None => {
                let initial_value =
                    state.get_initial_state().get_compiled_class_hash(class_hash)?;
    ...
    fn get_compiled_contract_class(&self, class_hash: ClassHash) -> StateResult<ContractClass> {
        ...
        match state.compiled_contract_classes.read(self.tx_index, class_hash) {
            ...
            None => {
                let contract_class =
                    state.get_initial_state().get_compiled_contract_class(class_hash);
