...
#[fixture]
pub fn versioned_state() -> Arc<Mutex<VersionedState<DictStateReader>>> {
...
#[rstest]
fn test_validate_read_set(versioned_state: Arc<Mutex<VersionedState<DictStateReader>>>) {
    let contract_address = contract_address!("0x1");
...
#[rstest]
fn test_apply_writes(versioned_state: Arc<Mutex<VersionedState<DictStateReader>>>) {
    let contract_address = contract_address!("0x1");
    let class_hash = ClassHash(stark_felt!(27_u8));
