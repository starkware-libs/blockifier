...
fn test_versioned_proxy_state_flow() {
    ...
    transactional_states[1].get_class_hash_at(contract_address).unwrap();
    transactional_states[1].get_nonce_at(contract_address).unwrap();

    transactional_states[1].get_actual_state_changes().unwrap();
    assert!(
        versioned_state
            .lock()
            .unwrap()
            .validate_read_set(1, transactional_states[1].cache.borrow().clone())
    );
    ...
    versioned_state.lock().unwrap().apply_writes(
        1,
        transactional_states[1].cache.borrow().clone(),
        transactional_states[1].class_hash_to_class.borrow().clone(),
    );
    ...
    transactional_states[2].set_class_hash_at(contract_address, class_hash_2).unwrap();
    ...
    for (i, transactional_state) in transactional_states.iter_mut().enumerate().take(4) {
        versioned_state.lock().unwrap().apply_writes(
            i.try_into().unwrap(),
            transactional_state.cache.borrow().clone(),
