...
fn test_validate_read_set(
    ...
    transactional_state.get_storage_at(contract_address_for_testing, storage_key).unwrap();
    ...
    transactional_state.get_nonce_at(contract_address_for_testing).unwrap();
    ...
    transactional_state.get_class_hash_at(contract_address_for_testing).unwrap();
    ...
    transactional_state.get_compiled_class_hash(class_hash_for_testing).unwrap();
...
fn test_apply_writes(
    ...
    transactional_states[0].set_class_hash_at(contract_address_for_testing, class_hash_0).unwrap();
    ...
    transactional_states[0]
        .set_contract_class(class_hash_for_testing, contract_class_0.clone())
        .unwrap();
    ...
    assert!(
        transactional_states[1].get_class_hash_at(contract_address_for_testing).unwrap()
            == class_hash_0
    );
    assert!(
        transactional_states[1].get_compiled_contract_class(class_hash_for_testing).unwrap()
            == contract_class_0
    );
}
