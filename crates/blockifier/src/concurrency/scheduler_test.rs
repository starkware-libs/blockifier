...
#[rstest]
fn test_lock_tx_status_no_panic() {
...
fn test_lock_tx_status_poisoned() {
    ...
    let status = arc_scheduler.lock_tx_status(0);
    assert_eq!(*status, TransactionStatus::ReadyToExecute);
}
...
#[should_panic(expected = "Transaction 0 is not executing")]
...
#[should_panic(expected = "We initiate re-execution in case of aborted transaction only. \
                           Transaction 0 is not aborting.")]
