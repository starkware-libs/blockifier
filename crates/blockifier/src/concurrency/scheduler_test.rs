...
#[rstest]
#[case::reduces_validation_index(0, TransactionStatus::Executing, 10)]
#[case::does_not_reduce_validation_index(10, TransactionStatus::Executing, 0)]
...
#[case::wrong_status_ready(0, TransactionStatus::ReadyToExecute, 0)]
#[should_panic(expected = "Only executing transactions can gain status executed. Transaction 0 \
                           is not executing. Transaction status: Executed.")]
#[case::wrong_status_executed(0, TransactionStatus::Executed, 0)]
#[should_panic(expected = "Only executing transactions can gain status executed. Transaction 0 \
                           is not executing. Transaction status: Aborting.")]
#[case::wrong_status_aborting(0, TransactionStatus::Aborting, 0)]
fn test_finish_execution(
    #[case] tx_index: usize,
    #[case] tx_status: TransactionStatus,
    #[case] validation_index: usize,
) {
    let n_active_tasks = 5;
    let scheduler = default_scheduler!(
        chunk_size: DEFAULT_CHUNK_SIZE,
        validation_index: validation_index,
        n_active_tasks: n_active_tasks,
...
#[rstest]
#[case::not_aborted(0, TransactionStatus::Executed, 10, false, None)]
...
#[case::does_not_return_validation_task(10, TransactionStatus::Aborting, 0, true, None)]
...
#[case::wrong_status_ready(0, TransactionStatus::ReadyToExecute, 0, true, None)]
#[should_panic(expected = "Only aborting transactions can be re-executed. Transaction 0 is not \
                           aborting. Transaction status: Executed.")]
#[case::wrong_status_executed(0, TransactionStatus::Executed, 0, true, None)]
#[should_panic(expected = "Only aborting transactions can be re-executed. Transaction 0 is not \
                           aborting. Transaction status: Executing.")]
#[case::wrong_status_executing(0, TransactionStatus::Executing, 0, true, None)]
