...
#[should_panic(expected = "Transaction not executing")]
...
fn test_try_validation_abort(
    ...
    let mut status = scheduler.tx_statuses[0].lock().unwrap();
    *status = target_status;
    drop(status);
    let result = scheduler.try_validation_abort(0);
    assert_eq!(result, expected_result);
    if result {
        assert_eq!(*scheduler.tx_statuses[0].lock().unwrap(), TransactionStatus::Aborting);
    }
}
...
#[should_panic(expected = "Transaction not aborting")]
...
fn test_finish_validation(
    ...
    *status = tx_status;
    ...
    assert_eq!(result, expected_result);
    let new_status = scheduler.tx_statuses[tx_index].lock().unwrap();
...
#[case::executed(1, Some(TransactionStatus::Executing), None, 0)]
...
#[case::executed(1, Some(TransactionStatus::Executing), None, 2, 0)]
