...
impl Scheduler {
    ...
    fn try_incarnate(&self, tx_index: TxIndex) -> Option<TxIndex> {
        if tx_index < self.chunk_size {
            let mut status = self.tx_statuses[tx_index]
                .lock()
                .unwrap_or_else(|_| panic!("Status of transaction index {tx_index} is poisoned."));
    ...
    fn next_version_to_validate(&self) -> Option<TxIndex> {
        ...
        if index_to_validate < self.chunk_size {
            let status = self.tx_statuses[index_to_validate].lock().unwrap_or_else(|_| {
                panic!("Status of transaction index {index_to_validate} is poisoned.")
            });
