...
impl Scheduler {
    ...
    pub fn next_version_to_validate(&self) -> Option<u32> {
        let mut index_to_validate = self.validation_index.load(Ordering::Acquire);
        if index_to_validate >= self.chunk_size {
            self.check_done();
            return None;
        }
        self.n_active_tasks.fetch_add(1, Ordering::SeqCst);
        index_to_validate = self.validation_index.fetch_add(1, Ordering::SeqCst);
        if index_to_validate < self.chunk_size {
    ...
    pub fn next_version_to_execute(&self) -> Option<u32> {
        let mut index_to_execute = self.execution_index.load(Ordering::Acquire);
        if index_to_execute >= self.chunk_size {
            self.check_done();
            return None;
        }
        self.n_active_tasks.fetch_add(1, Ordering::SeqCst);
        index_to_execute = self.execution_index.fetch_add(1, Ordering::SeqCst);
