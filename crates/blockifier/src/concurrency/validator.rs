    ...
    for read in read_set {
        match read.storage_type {
            StorageType::Storage => {
                ...
                let value = versioned_state.get_storage_at(version, arg.0, arg.1);
                assert!(value.is_ok());
                let read_value = read
                    .value
            ...
            StorageType::Nonce => {
                ...
                let value = versioned_state.get_nonce_at(version, *arg);
                assert!(value.is_ok());
                let read_value = read
                    .value
                    .downcast_ref::<Nonce>()
            ...
            StorageType::ClassHash => {
                let arg = read
                    ...
                    .downcast_ref::<ContractAddress>()
                    .expect("Failed to downcast read value to ContractAddress");
                let value = versioned_state.get_class_hash_at(version, *arg);
                assert!(value.is_ok());
                let read_value = read
                    .value
            ...
            StorageType::CompiledClassHash => {
                let arg = read
                    ...
                    .downcast_ref::<ClassHash>()
                    .expect("Failed to downcast read value to ClassHash");
                let value = versioned_state.get_compiled_class_hash(version, *arg);
                assert!(value.is_ok());
                let read_value = read
                    .value
