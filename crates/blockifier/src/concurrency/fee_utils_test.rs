...
pub fn test_fix_call_info(
    ...
    let (mut concurrency_call_info, concurrency_actual_fee) =
        create_fee_transfer_call_data(state, &account_tx, true);
    let call_info = create_fee_transfer_call_data(state, &account_tx, false).0;
    ...
    fix_concurrency_fee_transfer_call_info(
        ...
        StarkFelt::from(concurrency_actual_fee),
    );
