...
fn test_versioned_state() {
    ...
    let mut versioned_state = VersionedState::new(cached_state);
    ...
    let stark_felt_3 = stark_felt!(14_u8);
    let nonce_4 = Nonce(stark_felt!(3_u8));
    let class_hash_7 = ClassHash(stark_felt!(28_u8));
    let class_hash_10 = ClassHash(stark_felt!(29_u8));
    let compiled_class_hash_21 = CompiledClassHash(stark_felt!(30_u8));

    versioned_state.set_storage_at(3, contract_address, new_key, stark_felt_3);
    versioned_state.set_nonce_at(4, contract_address, nonce_4);
    versioned_state.set_class_hash_at(7, contract_address, class_hash_7);
    versioned_state.set_class_hash_at(10, contract_address, class_hash_10);
    versioned_state.set_compiled_class_hash(21, class_hash, compiled_class_hash_21);

    // Read the data
    assert_eq!(versioned_state.get_nonce_at(2, contract_address).unwrap(), nonce);
    assert_eq!(versioned_state.get_nonce_at(5, contract_address).unwrap(), nonce_4);

    assert_eq!(versioned_state.get_storage_at(5, contract_address, key).unwrap(), stark_felt);
    assert_eq!(versioned_state.get_storage_at(5, contract_address, new_key).unwrap(), stark_felt_3);

    assert_eq!(versioned_state.get_class_hash_at(2, contract_address).unwrap(), class_hash);
    assert_eq!(versioned_state.get_class_hash_at(9, contract_address).unwrap(), class_hash_7);
    assert_eq!(versioned_state.get_class_hash_at(10, contract_address).unwrap(), class_hash_10);
