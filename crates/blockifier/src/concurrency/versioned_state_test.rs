...
fn test_false_validate_reads(
    ...
    // Check compiled contract classes validation.
    let version_state_proxy_8 = safe_versioned_state.pin_version(8);
    let mut version_state_proxy_9 = safe_versioned_state.pin_version(9);
    let transactional_state_9 =
        TransactionalState::create_transactional(&mut version_state_proxy_9);

    let class_hash = class_hash!(2_u8);
    let contract_class = FeatureContract::TestContract(CairoVersion::Cairo1).get_class();
    assert_matches!(
        transactional_state_9.get_compiled_contract_class(class_hash).unwrap_err(),
        StateError::UndeclaredClassHash(class_hash) if
        class_hash == class_hash
    );
    version_state_proxy_8.state().apply_writes(
        8,
        &StateMaps {
            declared_contracts: HashMap::from([(class_hash, true)]),
            ..Default::default()
        },
        &HashMap::from([(class_hash, contract_class.clone())]),
    );
    assert!(
        !safe_versioned_state
            .pin_version(9)
            .validate_reads(&transactional_state_9.cache.borrow().initial_reads)
    );

    // Check declared contracts validation.
