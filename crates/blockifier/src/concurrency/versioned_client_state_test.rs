...
pub fn build_client_states(
    ...
    for (contract, n_instances) in contract_instances.iter() {
        ...
        for instance in 0..*n_instances {
            let instance_address = contract.get_instance_address(instance.into());
            address_to_class_hash.insert(instance_address, class_hash);
        }
    }

    Arc::new(Mutex::new(VersionedState::new(CachedState::from(DictStateReader {
        address_to_class_hash,
        class_hash_to_class,
        ..Default::default()
    }))))
...
fn test_run_parallel_txs() {
    ...
    // Initiate States
    let versioned_state =
        build_client_states(chain_info, &[(account_without_validation, 1), (grindy_account, 1)]);
    let mut state_1 = CachedState::from(VersionedClientState::new(1, Arc::clone(&versioned_state)));
    let mut state_2 = CachedState::from(VersionedClientState::new(1, Arc::clone(&versioned_state)));

    let _ = state_1.set_storage_at(
        chain_info.fee_token_address(&FeeType::Eth),
        get_fee_token_var_address(grindy_account.get_instance_address(0)),
        stark_felt!(BALANCE),
    );
    let _ = state_2.set_storage_at(
        chain_info.fee_token_address(&FeeType::Eth),
        get_fee_token_var_address(account_without_validation.get_instance_address(0)),
        stark_felt!(BALANCE),
    );
