...
pub fn test_commit_tx() {
    ...
    let transactions_array = [
        ...
        Transaction::AccountTransaction(trivial_call_data_transaction(account, 4_u16)),
    ];
    ...
    // create executer for commit tx test.
    ...
    for tx_index in 0..transactions_array.len() {
        if tx_index == 4 {
            change_account_balance(
                account,
                &mut executer.state.pin_version(tx_index - 1),
                4_u16,
        ...
        [(0, 0_u128, 0_u16), (1, transfer_ammount, 1_u16)]
    {
        ...
        executer.commit_tx(tx_index).unwrap();
