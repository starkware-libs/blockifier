...
fn test_worker_execute() {
    ...
    let storage_key = 1993_u16;

    let invoke_tx_success = invoke_tx(invoke_tx_args! {
        sender_address: account_address,
        calldata: create_calldata(
            test_contract_address,
            "test_storage_read_write",
            &[stark_felt!(storage_key),storage_value ], // Calldata:  address, value.
        ),
        max_fee: Fee(MAX_FEE),
        nonce: nonce_manager.next(account_address)
    });
    let tx_success = Transaction::AccountTransaction(AccountTransaction::Invoke(invoke_tx_success));

    let invoke_tx_failure = invoke_tx(invoke_tx_args! {
        sender_address: account_address,
        calldata: create_calldata(
            test_contract_address,
            "test_storage_read_write",
    ...
    let tx_failure = Transaction::AccountTransaction(AccountTransaction::Invoke(invoke_tx_failure));
    nonce_manager.rollback(account_address);

    let invoke_tx_revert = invoke_tx(invoke_tx_args! {
        sender_address: account_address,
        calldata: create_calldata(
            test_contract_address,
            "write_and_revert",
    ...
    let tx_revert = Transaction::AccountTransaction(AccountTransaction::Invoke(invoke_tx_revert));
    ...
    assert_eq!(
        safe_versioned_state
            ...
            .get_storage_at(test_contract_address, StorageKey(patricia_key!(storage_key)))
            .unwrap(),
        storage_value
    );
    ...
    let result = execution_output.as_ref().unwrap().result.as_ref().unwrap();
    ...
    let writes = StateMaps {
        ...
        storage: HashMap::from([
            ((test_contract_address, StorageKey(patricia_key!(storage_key))), storage_value),
    ...
    let reads = StateMaps {
        ...
        storage: HashMap::from([
            ((test_contract_address, StorageKey(patricia_key!(storage_key))), stark_felt!(0_u8)),
            ((erc_contract_address, account_balance_key_low), stark_felt!(BALANCE)),
            ((erc_contract_address, account_balance_key_high), stark_felt!(0_u8)),
        ]),
        // When running an entry point, we load its contract
    ...
    assert_eq!(execution_output.as_ref().unwrap().writes, writes);
    assert_eq!(execution_output.as_ref().unwrap().reads, reads);
    assert_ne!(execution_output.as_ref().unwrap().visited_pcs, HashMap::default());
    ...
    let execution_output = worker_executor.execution_outputs[tx_index].lock().unwrap();
    assert!(execution_output.as_ref().unwrap().result.as_ref().is_err());
    assert_eq!(execution_output.as_ref().unwrap().writes, StateMaps::default());
    assert_eq!(execution_output.as_ref().unwrap().visited_pcs, HashMap::default());
    ...
    assert_ne!(execution_output.as_ref().unwrap().writes, StateMaps::default());
    assert_ne!(execution_output.as_ref().unwrap().visited_pcs, HashMap::default());
