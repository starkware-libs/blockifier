...
fn change_account_balance(
    ...
    let mut version_state = state.pin_version(tx_index - 1);
    let account_balance_key_low =
...
fn check_pre_commit_execution<S: StateReader>(
    executor: &WorkerExecutor<'_, S>,
    tx_index: usize,
    account: FeatureContract,
    account_id: u16,
...
fn check_post_commit_execution<S: StateReader>(
    executor: &WorkerExecutor<'_, S>,
    tx_index: usize,
    account: FeatureContract,
    account_id: u16,
...
pub fn test_commit_tx() {
    ...
    let actual_fee = check_post_commit_execution(
        &executor,
        tx_index,
        account,
        account_id,
    ...
    // Flow3: failed execute -> pass re-validate -> commit.
    ...
    // Flow4: passed execute ->  failed re-validate ->  failed re-execute (rejected) -> commit.
    ...
    // Flow5: failed execute -> failed revalidate -> passed re-execute -> commit.
    ...
    executor.commit_tx(tx_index).unwrap();
    should_fail = false;
    let _actual_fee = check_post_commit_execution(
        &executor,
        tx_index,
        account,
        account_id,
