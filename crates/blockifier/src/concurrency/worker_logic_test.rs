...
fn pre_commit_tx_check<S: StateReader>(
    executor: &WorkerExecutor<'_, S>,
    tx_index: usize,
    account_address: ContractAddress,
...
fn post_commit_tx_check<S: StateReader>(
    executor: &WorkerExecutor<'_, S>,
    tx_index: usize,
    account_address: ContractAddress,
...
pub fn test_commit_tx() {
    ...
    let actual_fee = post_commit_tx_check(
        &executor,
        tx_index,
        account_address,
        account_balance,
    ...
    // This transaction transfers funds to the sequencer. We change the sequencer balance in the
    // write set, thus the transaction should fail the re-validate in the commit phase but pass
    // the re-execute.
    ...
    executor.commit_tx(tx_index).unwrap();
    should_fail = false;
    let _actual_fee = post_commit_tx_check(
        &executor,
        tx_index,
        account_address,
        account_balance,
