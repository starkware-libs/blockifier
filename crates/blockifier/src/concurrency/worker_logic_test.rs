...
fn test_worker_execute() {
    ...
    // Create the state.
    let cached_state =
        test_state(chain_info, BALANCE, &[(account_contract, 1), (test_contract, 1)]);
    let safe_versioned_state = safe_versioned_state_for_testing(cached_state);
...
fn test_deploy_before_declare() {
    ...
    let chain_info = &block_context.chain_info;
    let account_contract = FeatureContract::AccountWithoutValidations(CairoVersion::Cairo1);
    let cached_state = test_state(chain_info, BALANCE, &[(account_contract, 2)]);
    let safe_versioned_state = safe_versioned_state_for_testing(cached_state);
