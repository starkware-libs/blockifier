...
use crate::transaction::constants::{
    DEPLOY_CONTRACT_FUNCTION_ENTRY_POINT_NAME, TRANSFER_ENTRY_POINT_NAME,
};
...
fn trivial_transfer_tx(
    ...
    let transfer_calldata = create_calldata(
        ...
        &[
            *account_address.0.key(), // Calldata: recipient.
            stark_felt!(0_u8),        // Calldata: lsb amount.
...
pub fn test_commit_tx() {
    ...
    // tx1 should fail execution since it has nonce equals 1, and it is being executed before tx0,
    // which has nonce eqauls 0.
    // tx0 should pass execution.
    // tx2 should pass execution since it has nonce equals 1, so executing it after tx0 should
    // succeed.
    // tx3 should fail execution regardless of execution order, since it has nonce
    ...
    //   and commit.
    // * tx1 should fail re-validation (it read the noce before tx0 incremented it). It should pass
    //   re-execution (since tx0 incremented the nonce), fix the sequencer balance, fix the call
    //   info (fee transfer) and commit.
    // * tx2 should fail re-validation (it read the noce before tx1 re-executed and incremented it.
