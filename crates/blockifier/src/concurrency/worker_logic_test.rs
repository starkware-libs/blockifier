...
pub fn test_add_fee_to_sequencer_balance(
    ...
    let expected_sequencer_balance_value_low =
        stark_felt_to_felt(sequencer_balance_low) + Felt252::from(actual_fee.0);
    let overflow = sequencer_balance_low
        > felt_to_stark_felt(&(Felt252::max_value() - Felt252::from(actual_fee.0)));
    let expected_value_high = if overflow {
        stark_felt_to_felt(sequencer_balance_high) + Felt252::from(1_u128)
    } else {
        stark_felt_to_felt(sequencer_balance_high)
    };
