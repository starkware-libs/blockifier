...
impl<S: StateReader> State for CachedState<S> {
    ...
    fn set_contract_class(
        ...
        let mut cache = self.cache.borrow_mut();
        cache.declare_contract(class_hash, true);
        Ok(())
    }
...
impl StateCache {
    fn declare_contract(&mut self, class_hash: ClassHash, is_declared: bool) {
        self.declared_contract_writes.insert(class_hash, is_declared);
    }
