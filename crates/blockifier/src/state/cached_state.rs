...
impl StateChanges {
    ...
    pub fn merge(state_changes: &[&Self]) -> Self {
        let mut merged_state_changes = Self::default();
        for state_change in state_changes {
            merged_state_changes.storage_updates.extend(state_change.storage_updates.clone());
            merged_state_changes.class_hash_updates.extend(state_change.class_hash_updates.clone());
            merged_state_changes
                .compiled_class_hash_updates
                .extend(state_change.compiled_class_hash_updates.clone());
            merged_state_changes.modified_contracts.extend(state_change.modified_contracts.clone());
        }
