...
impl<S: StateReader> CachedState<S> {
    ...
    /// Creates a `TransactionalState` for the given `state`.
    ///
    /// This function provides a transactional interface to perform buffered `write` actions on the
    /// `state`. The `TransactionalState` has `commit` and `abort` methods for finalizing or
