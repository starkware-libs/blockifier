...
impl From<StateChanges> for StateChangesCount {
    fn from(value: StateChanges) -> Self {
        Self {
            n_storage_updates: value.storage_updates.len(),
            n_class_hash_updates: value.class_hash_updates.len(),
            n_compiled_class_hash_updates: value.compiled_class_hash_updates.len(),
            n_modified_contracts: value.modified_contracts.len(),
        }
    }
}
