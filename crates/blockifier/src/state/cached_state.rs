...
impl<S: StateReader> CachedState<S> {
    ...
    pub fn get_actual_state_changes_for_fee_charge(
        ...
        // Relevant only in account transactions.
        if let Some(sender_address) = sender_address {
            let sender_balance_key = get_erc20_balance_var_address(&sender_address)?;
            // Add the account balance change to storage updates. Default value is zero, which must
            // be different from the initial balance, otherwise the transaction would have failed.
