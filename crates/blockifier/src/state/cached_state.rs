...
pub struct TransactionalState<'a, U: UpdatableState> {
    pub state: CachedState<MutRefState<'a, U>>,
}
...
impl<'a, U: UpdatableState> TransactionalState<'a, U> {
    ...
    pub fn create_transactional(state: &mut U) -> TransactionalState<'_, U> {
        TransactionalState { state: CachedState::new(MutRefState::new(state)) }
    }

    /// Commits changes in the child (wrapping) state to its parent.
    pub fn commit(self) {
        let state = self.state.state.0;
        let child_cache = self.state.cache.into_inner();
        state.apply_writes(
            child_cache.writes,
            self.state.class_hash_to_class.into_inner(),
            &self.state.visited_pcs,
        )
    }
...
impl<'a, U: UpdatableState> UpdatableState for TransactionalState<'a, U> {
    fn apply_writes(
        ...
        self.state.apply_writes(writes, class_hash_to_class, visited_pcs)
    }
}
