...
impl StateCache {
    ...
    fn get_revertible_storage_updates(
        &self,
        validation_state_cache: &StateCache,
    ) -> HashMap<StorageEntry, StarkFelt> {
        let mut storage_writes = validation_state_cache.storage_writes.clone();
...
impl<'a, S: StateReader> TransactionalState<'a, S> {
    ...
    pub fn get_actual_revertible_state_changes(&mut self) -> StateResult<StateChanges> {
        self.update_initial_values_of_write_only_access()?;
        let storage_changes =
            self.cache.borrow().get_revertible_storage_updates(&self.state.0.cache.borrow());
        self.get_actual_state_changes_given_storage_updates(storage_changes)
    }
