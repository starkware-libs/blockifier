...
fn test_state_changes_merge(
    ...
    // The hash map 'storage_updates' is unordered, so we make sure we use the correct contract
    // address.
    let (contract_address, storage_key) = {
        let (contract_address, storage_key) = *storage_updates_keys.next().unwrap();
        if contract_address == contract_address!(CONTRACT_ADDRESS) {
            (contract_address, storage_key)
        } else if let Some(&(contract_address, storage_key)) = storage_updates_keys.next() {
            assert_eq!(contract_address, contract_address!(CONTRACT_ADDRESS));
            (contract_address, storage_key)
        } else {
            panic!(
