...
fn test_state_changes_merge() {
    ...
    assert_eq!(StateChanges::merge(&vec![&state_changes1, &state_changes2]), state_changes1);
    assert_eq!(StateChanges::merge(&vec![&state_changes2, &state_changes1]), state_changes1);

    // Get the storage updates' addresses and keys from the state_changes1, to overwrite.
    let mut storeage_updates_keys = state_changes1.storage_updates.keys().into_iter();
    let (contract_address, storage_key) = storeage_updates_keys.next().unwrap().clone();
    let (contract_address2, storage_key2) = storeage_updates_keys.next().unwrap().clone();
    ...
    let state_changes_final = state
        ...
        .unwrap();
    assert_eq!(
        StateChanges::merge(&vec![&state_changes1, &state_changes2, &state_changes3]),
        state_changes_final
    );
    assert_ne!(
