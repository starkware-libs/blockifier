...
fn create_state_changes_for_test<S: StateReader>(state: &mut CachedState<S>) -> StateChanges {
    ...
    // TODO(Zuphit, 1/9/2023): NEW_TOKEN_SUPPORT take correct token address by tx version.
...
fn test_state_changes_merge() {
    ...
    let block_context = BlockContext::create_for_testing();
    // TODO(Zuphit, 1/9/2023): NEW_TOKEN_SUPPORT consider generalizing test for multiple tx versions
