...
use blockifier::state::cached_state::{
    CachedState, CommitmentStateDiff, StagedTransactionalState, StateWriteKeys, StorageEntry,
    TransactionalState,
};
...
pub struct TransactionExecutor<S: StateReader> {
    ...
    // This member should (and must) be consistent with the state's write keys.
    // Kept in separate since the cache's members are private.
    pub state_write_keys: StateWriteKeys,
...
impl<S: StateReader> TransactionExecutor<S> {
    pub fn new(state: CachedState<S>, block_context: BlockContext) -> NativeBlockifierResult<Self> {
        ...
        let tx_executor = Self {
            ...
            state_write_keys: state.cache.get_write_keys(),
            state,
    ...
    pub fn execute(
        ...
        match tx_execution_result {
            Ok(tx_execution_info) => {
                ...
                // state and execution info rather than the cumulative state attributes.
                ...
                let tx_state_write_keys = transactional_state.cache.get_write_keys();
                let tx_unique_write_keys = tx_state_write_keys.difference(&self.state_write_keys);
                let state_diff_size = get_onchain_data_segment_length(tx_unique_write_keys.count());
    ...
    pub fn commit(&mut self) {
        ...
        self.state_write_keys.extend(&finalized_transactional_state.tx_unique_write_keys);
