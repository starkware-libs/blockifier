...
impl<S: StateReader> TransactionExecutor<S> {
    ...
    pub fn finalize(&mut self, is_pending_block: bool) -> (PyStateDiff, Vec<(PyFelt, Vec<usize>)>) {
        ...
        // Extract used_pcs from block_context, and convert it to a python-friendly type.
        let used_pcs_vec = self
            .state
            .used_pcs
            .iter()
            .map(|(class_hash, class_used_pcs)| {
                let mut class_used_pcs_vec: Vec<_> = class_used_pcs.iter().cloned().collect();
                class_used_pcs_vec.sort();
                (PyFelt::from(*class_hash), class_used_pcs_vec)
            })
            .collect();

        (PyStateDiff::from(self.state.to_state_diff()), used_pcs_vec)
    }
    ...
    pub fn commit(&mut self) {
        ...
        self.state.update_used_pc_cache(&finalized_transactional_state.used_pcs);
