...
impl<S: StateReader> TransactionExecutor<S> {
    ...
    pub fn execute(
        ...
        match tx_execution_result {
            Ok(tx_execution_info) => {
                ...
                // Note: cancelling writes (0 -> 1 -> 0) will not be removed, but it's fine
                // since someone paid for those keys.
                let tx_state_changes_keys =
                ...
                let state_diff_size =
                    get_onchain_data_segment_length(tx_state_changes_keys.count());
    ...
    pub fn commit(&mut self) {
        ...
        self.state_changes_keys.extend(&finalized_transactional_state.tx_state_changes_keys);
