...
pub struct TransactionExecutor<S: StateReader> {
    ...
    state_write_keys: StateWriteKeys,
...
impl<S: StateReader> TransactionExecutor<S> {
    pub fn new(state: CachedState<S>, block_context: BlockContext) -> NativeBlockifierResult<Self> {
        ...
        let tx_executor = Self {
            ...
            state_write_keys: StateWriteKeys::default(),
            state,
    ...
    pub fn execute(
        ...
        match tx_execution_result {
            Ok(tx_execution_info) => {
                ...
                let tx_state_write_keys = transactional_state.get_write_keys();
                let tx_unique_write_keys = tx_state_write_keys.difference(&self.state_write_keys);
                // Note that block-constant felt are not counted here so the bouncer needs to
