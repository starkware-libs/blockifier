...
pub struct TransactionExecutor<S: StateReader> {
    ...
    // Note: Cancelling writes (0 -> 1 -> 0) will be counted (at most once per key).
...
impl<S: StateReader> TransactionExecutor<S> {
    ...
    pub fn commit(&mut self) {
        ...
        self.state_changes_keys.extend(&finalized_transactional_state.tx_unique_write_keys);
