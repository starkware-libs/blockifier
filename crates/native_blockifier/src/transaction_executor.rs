...
use crate::py_transaction_execution_info::{PyBouncerInfo, PyVmExecutionResources};
use crate::py_utils::PyFelt;

pub type RawTransactionExecutionInfo = Vec<u8>;

#[pyclass]
#[derive(Debug, Serialize)]
pub struct TypedTransactionExecutionInfo {
    #[serde(flatten)]
    ...
    pub fn execute(
        ...
        match tx_execution_result {
            Ok(tx_execution_info) => {
                ...
                let typed_tx = TypedTransactionExecutionInfo {
                    info: tx_execution_info,
                    tx_type: tx_type.to_string(),
                };

                let raw_typed_tx_execution_info = serde_json::to_string(&typed_tx)
                    .map(|s| s.into_bytes())
                ...
                Ok((raw_typed_tx_execution_info, py_bouncer_info))
            }
            Err(error) => {
