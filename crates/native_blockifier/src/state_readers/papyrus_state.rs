...
pub struct PapyrusReader {
    ...
    pub transaction_timer_list: Vec<PapyrusReaderTimer>,
...
impl PapyrusReader {
    pub fn new(storage_reader: StorageReader, latest_block: BlockNumber) -> Self {
        Self {
            storage_reader,
            latest_block,
            timer: PapyrusReaderTimer::default(),
            transaction_timer_list: Vec::new(),
            total_timer: PapyrusReaderTimer::default(),
        }
    }
    ...
    pub fn add_timer_to_list(&mut self, timer: PapyrusReaderTimer) {
        self.transaction_timer_list.push(timer);
    }
}
...
impl fmt::Display for PapyrusReader {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Display transaction_timer_list
        writeln!(f, "Time spent for each transaction in Papyrus:")?;
        for timer in &self.transaction_timer_list {
            writeln!(f, "\n{}", timer)?;
        }

        // Display total_timer
        writeln!(f, "\nTime spent for block in Papyrus: \n{}", self.total_timer)?;
