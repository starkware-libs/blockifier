...
impl PyBouncerInfo {
    pub fn calculate(
        ...
        let builtin_instance_counter: HashMap<String, usize> = builtin_ordered_list
            .iter()
            .map(|name| {
                (name.name().to_string(), *tx_actual_resources.0.get(name.name()).unwrap_or(&0))
            })
            .collect();

        let actual_resources_execution_resources = VmExecutionResources {
            n_steps: *tx_actual_resources.0.get(constants::N_STEPS_RESOURCE).unwrap_or(&0),
            n_memory_holes: *tx_actual_resources.0.get("n_memory_holes").unwrap_or(&0),
            builtin_instance_counter,
        };

        let mut merged_resources =
            tx_additional_os_resources.add(&actual_resources_execution_resources);
