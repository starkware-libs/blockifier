...
impl TryFrom<PyInvokeTransactionV0> for InvokeTransactionV0 {
    ...
    fn try_from(tx: PyInvokeTransactionV0) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            calldata: Calldata(from_py_felts(tx.calldata).into()),
        })
    }
}
...
pub fn py_invoke_function(py_tx: &PyAny) -> NativeBlockifierResult<InvokeTransaction> {
    ...
    let tx = match version {
        0 => {
            let py_declare_tx: PyInvokeTransactionV0 = py_tx.extract()?;
            let declare_tx = InvokeTransactionV0::try_from(py_declare_tx)?;
            Ok(starknet_api::transaction::InvokeTransaction::V0(declare_tx))
        }
        1 => {
            let py_declare_tx: PyInvokeTransactionV1 = py_tx.extract()?;
            let declare_tx = InvokeTransactionV1::try_from(py_declare_tx)?;
            Ok(starknet_api::transaction::InvokeTransaction::V1(declare_tx))
        }
        3 => {
            let py_declare_tx: PyInvokeTransactionV3 = py_tx.extract()?;
            let declare_tx = InvokeTransactionV3::try_from(py_declare_tx)?;
            Ok(starknet_api::transaction::InvokeTransaction::V3(declare_tx))
        }
