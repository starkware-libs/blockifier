...
impl PyValidator {
    ...
    pub fn perform_validations(
        ...
        // TODO(Gilad): make it so we don't have to pass both tx and account_tx, without
        // duplication.
        if !self.perform_pre_validation_checks(
            tx,
            &account_tx,
            raw_contract_class,
            deploy_account_tx_hash,
        )? {
            // Skip validation/post-validation if pre-validation checks failed.
            return Ok(());
        }
...
impl PyValidator {
    ...
    // All checks that should be done before executing `__validate__`.
    // Returns true if the transaction should proceed into `__validate__` execution.
    fn perform_pre_validation_checks(
        &mut self,
        tx: &PyAny,
        ...
        let account_tx_context = account_tx.get_account_tx_context();
        if self
            .skip_due_to_unprocessed_deploy_account(&account_tx_context, deploy_account_tx_hash)?
        {
            return Ok(false);
        }

        // Deploy account transactions should be fully executed (not just validated),
        // since the constructor must run before any validations are performed. Since the execution
        // already includes all necessary validations, so they are skipped here.
    ...
    fn skip_due_to_unprocessed_deploy_account(
        &mut self,
