...
native_blockifier_errors!(
    ...
    (
        NativeBlockifierValidateError,
        NativeBlockifierValidationsError,
        PyNativeBlockifierValidateError
    ),
...
#[derive(Debug, Error)]
pub enum NativeBlockifierValidationsError {
    #[error(
        "`{entry_point_name}` call failed; failure reason: {}.",
        format_panic_data(error_data)
    )]
    ValidationError { entry_point_name: String, error_data: Vec<StarkFelt> },
}
