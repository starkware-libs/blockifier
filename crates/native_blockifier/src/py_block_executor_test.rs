...
fn get_block_id() {
    ...
    let max_class_hash_as_py_felt = PyFelt(
        StarkFelt::new([
    ...
    let block_executor = PyBlockExecutor::create_for_testing_with_storage(
        Box::new(storage),
        PyGeneralConfig::default(),
    );
    assert_eq!(block_executor.get_block_id_at_target(0).unwrap(), None);

    assert_eq!(
        block_executor.get_block_id_at_target(1138).unwrap().unwrap(),
        max_class_hash_as_py_felt
    );
}
