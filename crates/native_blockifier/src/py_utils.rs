...
use blockifier::transaction::errors::{TransactionExecutionError, FeeCalculationError};
...
pub fn raise_error_for_testing() -> NativeBlockifierResult<()> {
    Err(TransactionExecutionError::FeeCalculationError(FeeCalculationError::CairoResourcesNotContainedInFeeCosts).into())
}
