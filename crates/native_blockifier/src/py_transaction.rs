...
fn get_v3_new_parameters(tx: &PyAny) -> V3NewParameters {
    let py_resource_bounds: PyResourceBounds = py_attr(tx, "resource_bounds")?;
    let py_nonce_data_availability_mode: PyDataAvailabilityMode = py_attr(tx, "nonce_data_availability_mode")?;
    let py_fee_data_availability_mode: PyDataAvailabilityMode = py_attr(tx, "fee_data_availability_mode")?;
    let py_fee_data_availability_mode: PyDataAvailabilityMode = py_attr(tx, "fee_data_availability_mode")?;
    V3NewParameters {
        tip: Tip(py_attr::<u64>(tx, "tip")?),
        resource_bounds: ResourceBounds::from(py_resource_bounds),
        nonce_data_availability_mode: DataAvailabilityMode::try_from(py_nonce_data_availability_mode)?,
