...
impl PyTransactionExecutor {
    ...
    pub fn execute(
        ...
        self.with_mut(|executor| {
            ...
            if let Err(error) = tx_execution_result {
                transactional_state.abort();
                return Err(error);
            }

            let py_tx_execution_info: PyTransactionExecutionInfo =
                tx_execution_result.unwrap().into();
            let args = (py_tx_execution_info.clone(),);
            // This is a callback to Python code.
            let has_enough_room_for_tx = enough_room_for_tx.call1(args);
