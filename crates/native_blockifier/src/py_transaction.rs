...
impl PyTransactionExecutor {
    ...
    pub fn pre_process_block(&mut self, n_from_current_block_number_and_hash: &PyAny) {
...
impl PyTransactionExecutorInner {
    ...
    pub fn pre_process_block(&mut self, n_from_current_block_number_and_hash: &PyAny) {
        ...
        let block_number = py_item(n_from_current_block_number_and_hash, 0).unwrap();
        let block_hash =
            biguint_to_felt(py_item(n_from_current_block_number_and_hash, 1).unwrap()).unwrap();
