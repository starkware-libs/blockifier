...
impl PyTransactionExecutorInner {
    ...
    pub fn pre_process_block(
        &mut self,
        n_from_current_block_number_and_hash: Option<(u64, PyFelt)>,
    ) -> NativeBlockifierResult<()> {
        if let Some((block_number, block_hash)) = n_from_current_block_number_and_hash {
            let hashhh = block_hash.0;
            log::debug!("pre_process_block Got: {block_number} {hashhh}");
            self.with_mut(|executor| {
                pre_process_block(
                    executor.state,
                    Some((BlockNumber(block_number), BlockHash(block_hash.0))),
                );
            });
        }
