...
fn get_common_transaction_fields(tx: &PyAny) -> NativeBlockifierResult<CommonTransactionFields> {
...
pub fn py_declare(
    ...
    let sn_api_tx = match version {
        ...
        3 => {
            let compiled_class_hash =
                CompiledClassHash(py_attr::<PyFelt>(tx, "compiled_class_hash")?.0);
            let common_transaction_fields = get_common_transaction_fields(tx)?;
            let py_account_deployment_data: Vec<PyFelt> = py_attr(tx, "account_deployment_data")?;
            let declare_tx = DeclareTransactionV3 {
                resource_bounds: common_transaction_fields.resource_bounds,
                tip: common_transaction_fields.tip,
                signature: account_data_context.signature,
                nonce: account_data_context.nonce,
                class_hash,
                compiled_class_hash,
                sender_address: account_data_context.sender_address,
                nonce_data_availability_mode: common_transaction_fields
                    .nonce_data_availability_mode,
                fee_data_availability_mode: common_transaction_fields.fee_data_availability_mode,
                paymaster_address: common_transaction_fields.paymaster_address,
