...
pub fn py_tx(tx: &PyAny, raw_contract_class: Option<&str>) -> NativeBlockifierResult<Transaction> {
    ...
    let account_tx = match tx_type {
        TransactionType::Declare => {
            let raw_contract_class: &str = raw_contract_class
                .expect("A contract class must be passed in a Declare transaction.");
            AccountTransaction::Declare(py_declare(tx, raw_contract_class)?)
        }
        TransactionType::DeployAccount => AccountTransaction::DeployAccount(py_deploy_account(tx)?),
        TransactionType::InvokeFunction => AccountTransaction::Invoke(py_invoke_function(tx)?),
        TransactionType::L1Handler => {
            return Ok(Transaction::from(py_l1_handler(tx)?));
        }
    };

    Ok(Transaction::from(account_tx))
}
