...
#[pyclass]
// To access a field you must use `self.borrow_{field_name}()`. Alternately, you can borrow the
// whole object using `self.with()` (or `self.with_mut()` for a mutable borrow).
...
impl PyTransactionExecutor {
    ...
    pub fn create(general_config: &PyAny, block_info: &PyAny) -> NativeBlockifierResult<Self> {
        ...
        fn storage_tx_builder(
            ...
            storage_reader
                .begin_ro_txn()
                .expect("Failed to create a read-only transaction for the storage reader.")
        }
        ...
        fn state_builder<'a>(
            ...
            let state_reader =
                storage_tx.get_state_reader().expect("Failed to get the state reader.");
        ...
        let py_tx_executor_builder = PyTransactionExecutorBuilder {
            block_context,
            storage_reader,
            storage_tx_builder,
            state_builder: |storage_tx| state_builder(storage_tx, block_number),
        };
        Ok(py_tx_executor_builder.build())
    }
