...
impl PyBlockExecutor {
    ...
    pub fn finalize(&mut self) -> (PyStateDiff, PyBlockInfo) {
        log::debug!("Finalizing execution...");
        let (py_state_diff, py_block_info) = self.tx_executor().finalize();
        log::debug!("Finalized execution.");

        self.end_block();

        (py_state_diff, py_block_info)
    }
