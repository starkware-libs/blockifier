...
use blockifier::block::{BlockInfo, BlockNumberHashPair, GasPrices};
use blockifier::context::{BlockContext, ChainInfo, FeeTokenAddresses};
use blockifier::execution::block_executor::{
    pre_process_block as pre_process_block_blockifier, versioned_constants_with_overrides,
    BlockExecutor,
};
use blockifier::execution::transaction_executor::TransactionExecutor;
...
// Happy flow:
// 1. Converting the Pythonic arguments for the BlockExecutor constructor to Rust arguments.
// 2. Creating a state reader aligned with the Pythonic Aerospike storage (setup_block_execution).
// 3. Creating an instance of BlockExecutor (setup_block_execution).
// 4. Calling the methods from Python and converting the rust return types of the BlockExecutor
//    methods to Pythonic types.

#[pyclass]
pub struct PyBlockExecutor {
    pub general_config: PyGeneralConfig,
    pub versioned_constants: VersionedConstants,
    pub block_executor: Option<BlockExecutor<PapyrusReader>>,
...
impl PyBlockExecutor {
    ...
    pub fn setup_block_execution(
        ...
        self.block_executor = Some(block_executor);
        Ok(())
    }
    ...
    pub fn execute(
        ...
        let (tx_execution_info, bouncer_info) = self.block_executor().execute_tx(tx, charge_fee)?;
    ...
    pub fn finalize(&mut self, is_pending_block: bool) -> (PyStateDiff, Vec<(PyFelt, Vec<usize>)>) {
        ...
        let (commitment_state_diff, visited_pcs) = self.block_executor().finalize(is_pending_block);
    ...
    pub fn commit_tx(&mut self) {
        self.block_executor().commit_tx()
    }

    pub fn abort_tx(&mut self) {
        self.block_executor().abort_tx()
    }
    ...
    pub fn revert_block(&mut self, block_number: u64) -> NativeBlockifierResult<()> {
        ...
        self.block_executor().global_contract_cache.clear(); ////////////////////////////////////////////////////////////////////////////////??????
...
impl PyBlockExecutor {
    fn block_executor(&mut self) -> &mut BlockExecutor<PapyrusReader> {
        self.block_executor.as_mut().expect("Transaction executor should be initialized")
    }
...
pub fn into_block_context_args(
    ...
    let block_info = BlockInfo {
        ...
        gas_prices: GasPrices {
            eth_l1_gas_price: py_block_info.l1_gas_price.price_in_wei.try_into().map_err(|_| {
                NativeBlockifierInputError::InvalidNativeBlockifierInputError(
                    InvalidNativeBlockifierInputError::InvalidGasPriceWei(
                        py_block_info.l1_gas_price.price_in_wei,
                    ),
                )
            })?,
            strk_l1_gas_price: py_block_info.l1_gas_price.price_in_fri.try_into().map_err(
                |_| {
                    NativeBlockifierInputError::InvalidNativeBlockifierInputError(
                        InvalidNativeBlockifierInputError::InvalidGasPriceFri(
                            py_block_info.l1_gas_price.price_in_fri,
                        ),
                    )
                },
            )?,
                ...
                .map_err(|_| {
                    NativeBlockifierInputError::InvalidNativeBlockifierInputError(
                        InvalidNativeBlockifierInputError::InvalidDataGasPriceWei(
                            py_block_info.l1_data_gas_price.price_in_wei,
                        ),
                    )
                })?,
                ...
                .map_err(|_| {
                    NativeBlockifierInputError::InvalidNativeBlockifierInputError(
                        InvalidNativeBlockifierInputError::InvalidDataGasPriceFri(
                            py_block_info.l1_data_gas_price.price_in_fri,
                        ),
                    )
                })?,
        },
