...
impl PyBlockExecutor {
    ...
    pub fn execute_txs(
        ...
        let tx_types: Vec<String> = txs_with_class_infos
            .iter()
            .map(|(tx, _)| get_py_tx_type(tx).expect(PY_TX_PARSING_ERR).to_string())
            .collect();
        ...
        Python::with_gil(|py| {
            let py_serialized_results: Vec<(bool, Py<PyBytes>)> = serialized_results
                ...
                .map(|(sucess, execution_result)| {
                    ...
                    (sucess, PyBytes::new(py, &execution_result).into())
                })
                .collect();
